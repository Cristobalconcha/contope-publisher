 ...
 * que el puente (tools/ocd-headless-node-edit.mjs) no puede correr allá. Este
 * programa hace el circuito completo desde acá:
 *
 *   1. Pide el documento real al sitio      (MCP: ocd_read_canvas_document)
 *   2. Se lo entrega al motor real de Grapes (puente headless, API de componentes)
 *   3. Guarda de vuelta lo que Grapes produjo (MCP: ocd_write_canvas_document)
 *
 ...
 *
 * Credenciales: se leen de ocd-grapes-runner.config.json (ver .example).
 * Ese archivo NO debe subirse al repositorio.
 *
 * Uso:
 *   node ocd-grapes-runner.mjs --page 44 --document ocd-canvas-page-14 \
 ...
 *   node ocd-grapes-runner.mjs ... --dry-run
 */
...
const bridgeScript = path.join(repoRoot, 'open-codesign-publisher', 'tools', 'ocd-headless-node-edit.mjs');
const buildScript = path.join(repoRoot, 'open-codesign-publisher', 'tools', 'ocd-headless-build.mjs');
const configPath = path.join(here, 'ocd-grapes-runner.config.json');
...
//   1. ocd-grapes-runner.config.json (lo habitual)
//   2. variables de entorno OCD_SITE_URL / OCD_WP_USER / OCD_WP_APP_PASSWORD
let config = {};
let origenConfig = '';
...
const desdeEntorno = {
  siteUrl: process.env.OCD_SITE_URL,
  username: process.env.OCD_WP_USER,
  applicationPassword: process.env.OCD_WP_APP_PASSWORD,
};
...
` +
    `  Opción A — archivo: copiá ocd-grapes-runner.config.example.json a
` +
    `             ${path.basename(configPath)} y completalo (se guarda, es una sola vez).
` +
    `  Opción B — entorno: definí OCD_SITE_URL, OCD_WP_USER y OCD_WP_APP_PASSWORD.`
  );
}
const endpoint = `${config.siteUrl.replace(/\/+$/, '')}/wp-json/open-codesign/v1/mcp`;
...
const doc = await callTool('ocd_read_canvas_document', { pageId, documentId });
...
// directo en el HTML guardado sin tocar projectData — ocd_patch_geo_places,
...
const ATRIBUTOS_DE_DATOS = [
  'data-ocd-geo-places',
  'data-ocd-shortcode-atts',
  'data-ocd-chart-data',
  'data-ocd-parcel-data',
];
...
// solo en la hoja guardada —las que se agregaron por ocd_patch_css, por
...
const saved = await callTool('ocd_write_canvas_document', {
  pageId,
  documentId,
