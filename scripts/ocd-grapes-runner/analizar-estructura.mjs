 ...
 * elementos van a recibir identidad estructural (ocd-section / ocd-columns /
 * ocd-column) y dónde el mapeo es dudoso y conviene decidirlo a mano.
 *
 * Uso:
...
config.siteUrl = config.siteUrl || process.env.OCD_SITE_URL;
config.username = config.username || process.env.OCD_WP_USER;
config.applicationPassword = config.applicationPassword || process.env.OCD_WP_APP_PASSWORD;
...
const res = await fetch(`${config.siteUrl.replace(/\/+$/, '')}/wp-json/open-codesign/v1/mcp`, {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', Accept: 'application/json', Authorization: auth },
  body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'tools/call', params: { name: 'ocd_read_canvas_document', arguments: { pageId, documentId } } }),
});
...
// ---- Vocabulario del sistema (verificado en ocd-editor-core.js / ocd-behaviors.js)
const YA_TIPADO = { 'ocd-section': 'SECCIÓN', 'ocd-columns': 'FILA', 'ocd-column': 'COLUMNA', 'ocd-group': 'GRUPO' };
...
const esModulo = (el) => el.hasAttribute('data-ocd-behavior') && MODULOS.has(el.getAttribute('data-ocd-behavior'));
...
function proponer(el, rol) {
  ...
  if (esModulo(el)) return { tipo: 'MÓDULO:' + el.getAttribute('data-ocd-behavior'), yaEstaba: true };
