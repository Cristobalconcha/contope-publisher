/**
 * Cambia el Marcador (el id) de un bloque del sitio, con el Grapes real.
 *   node scripts/renombrar-marcador.mjs "<selector>" "<nombre-nuevo>" [--aplicar]
 * Respalda siempre antes de escribir.
 */
...
const [selector, nombre] = args.filter((a) => !a.startsWith('--'));
if (!selector || !nombre) { console.error('uso: node scripts/renombrar-marcador.mjs "<selector>" "<nombre>" [--aplicar]'); process.exit(1); }
if (!/^[a-z][a-z0-9_-]*$/.test(nombre)) { console.error('el nombre debe empezar con letra y llevar solo minúsculas, números, - y _'); process.exit(1); }
...
const PÁGINA = 43, DOCUMENTO = 'ocd-canvas-page-7';
console.log('\n1/4  Leyendo…');
const doc = await llamar('cod_read_canvas_document', { pageId: PÁGINA, documentId: DOCUMENTO });
console.log(`     revisión ${doc.revision}`);

// Un id ya usado haría que el enlace llegue a cualquiera de los dos, sin aviso.
if (new RegExp('id="' + nombre + '"').test(doc.html)) {
  console.error(`\nYa hay un bloque con el marcador "${nombre}" en esta página. Un destino de enlace tiene que ser único.`);
  process.exit(1);
}
...
console.log(`\n2/4  Poniéndole el marcador "${nombre}"…`);
const r = spawnSync('node', [path.join(raíz, 'contope-publisher', 'tools', 'cod-headless-node-edit.mjs')], {
  input: JSON.stringify({ document: { projectData: doc.projectData, html: doc.html, css: doc.css },
    selector, mutation: { attributes: { id: nombre } } }),
  encoding: 'utf8', maxBuffer: 300 * 1024 * 1024 });
const línea = (r.stdout || '').trim().split('\n').filter(Boolean).pop() || '';
let salida;
try { salida = JSON.parse(línea); } catch { console.error('sin respuesta:', (r.stderr || '').slice(-500)); process.exit(1); }
if (!salida.ok) { console.error('el puente rechazó:', salida.error); process.exit(1); }

console.log('\n3/4  Comprobando…');
const pruebas = [
  ['el marcador nuevo está', new RegExp('id="' + nombre + '"').test(salida.html)],
  ['está una sola vez', (salida.html.match(new RegExp('id="' + nombre + '"', 'g')) || []).length === 1],
  ['el CSS no se encogió', salida.css.length > doc.css.length * 0.9],
  ['el HTML no se encogió', salida.html.length > doc.html.length * 0.9],
  ['los otros marcadores siguen', salida.html.includes('id="ubicacion"') && salida.html.includes('id="mapa-interactivo"')],
];
let fallas = 0;
...
console.log('\n4/4  Guardando…');
const g = await llamar('cod_write_canvas_document', { pageId: PÁGINA, documentId: DOCUMENTO,
  expectedRevision: doc.revision, projectData: salida.projectData, html: salida.html, css: salida.css });
console.log(`     guardado · nueva revisión ${g.revision}`);
console.log(`\nEl enlace queda: ${config.siteUrl.replace(/\/+$/, '')}/#${nombre}`);
console.log(`Para volver atrás: node scripts/restaurar-respaldo.mjs "${path.relative(raíz, respaldo)}" --aplicar\n`);
