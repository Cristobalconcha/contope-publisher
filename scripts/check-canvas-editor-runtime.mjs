...
(async()=>{let phase='inicio';try{
  ...
  const componentsHidden=document.querySelector('.ocd-canvas-workspace').getAttribute('data-ocd-active-panel')==='components'&&window.getComputedStyle(document.getElementById('ocd-canvas-inspector')).display==='none';
  phase='pestaña inspector';
  document.querySelector('[data-ocd-side-panel="inspector"]').click();
  const inspectorVisible=document.querySelector('.ocd-canvas-workspace').getAttribute('data-ocd-active-panel')==='inspector'&&window.getComputedStyle(document.getElementById('ocd-canvas-inspector')).display!=='none';
  ...
  await waitForSelector('[data-ocd-header-tool="pin"]');
  await waitForSelector('[data-ocd-header-tool="scroll"]');
  const headerPanelFromChild=!!document.querySelector('.ocd-ci__header-state');
  const headerToolsOnObject=!!document.querySelector('[data-ocd-header-tool="entry"]')&&!!document.querySelector('[data-ocd-header-tool="scroll"]');
  phase='cambio de estado desde el objeto';
  const toolbarCommand=api.editor.getSelected().get('toolbar').find(tool=>tool.attributes?.['data-ocd-header-tool']==='scroll')?.command;
  ...
  const entryCommand=api.editor.getSelected().get('toolbar').find(tool=>tool.attributes?.['data-ocd-header-tool']==='entry')?.command;
  ...
  await wait(()=>nav.getEl().classList.contains('nav--scrolled'));
  const toolbarRestored=JSON.stringify(logoWrap.get('toolbar')??null)===logoToolbarBefore&&!JSON.stringify(hero.get('toolbar')??null).includes('ocd-header-state');
  const runtimeRestoredAfterExit=nav.getEl().classList.contains('nav--scrolled')&&!nav.getEl().hasAttribute('data-ocd-preview-scroll-state');
  ...
  await wait(()=>document.querySelector('[data-ocd-header-tool="pin"]'));
  document.querySelector('[data-ocd-header-tool="pin"]').dispatchEvent(new MouseEvent('mousedown',{bubbles:true}));
  ...
  const toolbarNotSerialized=!stored.projectData.includes('ocd-header-state')&&!stored.projectData.includes('data-ocd-header-tool');
...
}catch(error){document.documentElement.dataset.ocdRuntime='failed';document.documentElement.dataset.ocdRuntimeDetails=phase+': '+error.message+' '+JSON.stringify({readyState:document.readyState,publishedRoots:document.querySelectorAll('.ocd-canvas-published').length,adminOffset:document.getElementById('published-fixed')?.getAttribute('data-ocd-admin-bar-offset'),canvas:!!window.ocdCanvas?.editor,selectedAttributes:window.ocdCanvas?.editor?.getSelected()?.getAttributes?.(),navAttributes:window.ocdCanvas?.editor?.getWrapper?.().find?.('.nav')?.[0]?.getAttributes?.(),selectedToolbar:window.ocdCanvas?.editor?.getSelected()?.get?.('toolbar'),toolbarItems:document.querySelectorAll('.gjs-toolbar-item').length,toolbarHtml:document.querySelector('.gjs-toolbar')?.outerHTML,activePanel:document.querySelector('.ocd-canvas-workspace')?.getAttribute('data-ocd-active-panel'),inspectorDisplay:window.getComputedStyle(document.getElementById('ocd-canvas-inspector')).display,publicScript:Array.from(document.scripts).some(script=>script.src.includes('ocd-canvas-public.js'))});}})();
</script></body></html>`;
