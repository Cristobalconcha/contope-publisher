/**
 * cod-preload.js — retiene la página hasta que lo que se ve arriba está listo.
 *
...
(function () {
    ...
    function retirar() {
        ...
        raiz.classList.add('cod-precarga-lista');
        // La clase de transición se limpia sola, para no dejar rastro en el DOM.
        window.setTimeout(function () { raiz.classList.remove('cod-precarga-lista'); }, 900);
    }
    ...
    function videoConMascara() {
        var envoltorios = doc.querySelectorAll('[data-cod-luma-matte]');
        if (!envoltorios.length) return Promise.resolve();
        var faltan = [];
        for (var i = 0; i < envoltorios.length; i += 1) {
            if (!envoltorios[i].classList.contains('cod-luma-matte--listo')) faltan.push(envoltorios[i]);
        }
        if (!faltan.length) return Promise.resolve();
        return new Promise(function (listo) {
            var restantes = faltan.length;
            faltan.forEach(function (envoltorio) {
                envoltorio.addEventListener('cod-luma-matte-listo', function () {
                    restantes -= 1;
