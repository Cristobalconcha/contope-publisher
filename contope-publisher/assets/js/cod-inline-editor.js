/**
 * ContOpe Canvas — editor en línea sobre la página publicada (M3).
 *
...
(function (window, document) {
    ...
    function findWrapperModel(editor, kind) {
        ...
        for (var i = 0; i < models.length; i++) {
            ...
            if (attributes && attributes['data-cod-inline-region'] === kind) {
                return models[i];
            }
        }
        return null;
    ...
    /**
     * Arma el compuesto (3 wrappers top-level con `data-cod-inline-region`) y
    ...
    function buildComposite(editor, config) {
        ...
        parts.forEach(function (part) {
            ...
            wrappers.push({
                tagName: 'div',
                attributes: {
                    'data-cod-inline-region': part.kind,
                    'data-cod-region-label': part.label
                },
                components: markup
            });
            htmlParts.push(
                '<div data-cod-inline-region="' + part.kind + '" data-cod-region-label="' + part.label + '">' +
                regionFlatHtml(doc) +
                '</div>'
            );
        });
    ...
    function computeSplit(editor, config, built, kind) {
        ...
        for (var i = 0; i < wrappersJson.length; i++) {
            ...
            if (attributes['data-cod-inline-region'] === kind) {
    ...
    var statusNode = document.getElementById('cod-inline-status');
    var regionNode = document.getElementById('cod-inline-region');
    var saveButton = document.getElementById('cod-inline-save');
    var templatesLink = document.getElementById('cod-inline-templates');
    var historyButton = document.getElementById('cod-inline-history');
    ...
    function setStatus(message, kind) {
        ...
        statusNode.className = 'cod-inline-status' + (kind ? ' is-' + kind : '');
    }
    ...
    var core = window.OCDEditorCore.create({
        container: '#cod-inline-canvas-root',
        status: setStatus,
        blocks: [],
     ...
     * `data-cod-inline-region` y devuelve su valor ('header'|'body'|'footer').
     */
    function findRegion(component) {
        var node = component;
        while (node) {
            var attributes = componentAttributes(node);
            if (attributes && attributes['data-cod-inline-region']) {
                return attributes['data-cod-inline-region'];
            }
    ...
    function isDynamicComponent(component) {
        ...
        if (attributes && attributes['data-cod-dynamic']) {
            return true;
        }
    ...
    function updateActiveRegion() {
        ...
        if (!kind) {
            ...
            if (regionNode) {
                ...
                regionNode.setAttribute('data-cod-region-kind', '');
                regionNode.classList.remove('is-dirty');
            }
            if (templatesLink) {
        ...
        if (regionNode) {
            ...
            regionNode.setAttribute('data-cod-region-kind', kind);
        }
    ...
    function sessionStorageKey() {
        return 'cod-inline-session-' + (config.pageId || 'page');
    }
    ...
    function setHistoryMessage(message, kind) {
        ...
        historyMessage.className = 'cod-inline-history-message' + (kind ? ' is-' + kind : '');
    }
    ...
    function historyEmptyNode(message) {
        ...
        node.className = 'cod-inline-history-empty';
    ...
    function disableHistoryItems(disabled) {
        ...
        var buttons = historyList.querySelectorAll('.cod-inline-history-restore');
    ...
    function closeHistory() {
        ...
        var history_button = document.getElementById('cod-inline-history');
        if (history_button) {
    ...
    function buildHistoryOverlay() {
        ...
        historyOverlay.className = 'cod-inline-history-overlay';
        historyOverlay.hidden = true;
        ...
        card.className = 'cod-inline-history-card';

        var header = document.createElement('div');
        header.className = 'cod-inline-history-header';

        var title = document.createElement('h2');
        title.className = 'cod-inline-history-title';
        title.textContent = 'Historial';

        var closeButton = document.createElement('button');
        closeButton.type = 'button';
        closeButton.className = 'cod-inline-history-close';
    ...
    function historyItem(entry) {
        ...
        item.className = 'cod-inline-history-item';

        var meta = document.createElement('div');
        meta.className = 'cod-inline-history-meta';

        var origin = document.createElement('span');
        origin.className = 'cod-inline-history-origin';
        origin.textContent = snapshotOrigin(entry);

        var label = document.createElement('span');
        label.className = 'cod-inline-history-label';
        label.textContent = snapshotLabel(entry);

        var date = document.createElement('span');
        date.className = 'cod-inline-history-date';
        ...
        restoreButton.className = 'cod-inline-history-restore';
    ...
    function openHistory() {
        ...
        var close_button = historyOverlay.querySelector('.cod-inline-history-close');
        if (close_button) {
            close_button.focus();
        }
