/**
 * ContOpe Canvas — editor experimental (entry de administración).
 *
 * Consume el núcleo compartido de `cod-editor-core.js` (window.OCDEditorCore)
...
(function (window, document) {
    ...
    var root = document.getElementById('cod-canvas-editor-root');
    var statusNode = document.getElementById('cod-canvas-status');
    ...
    function setStatus(message, kind) {
        ...
        statusNode.className = 'cod-canvas-status' + (kind ? ' is-' + kind : '');
    ...
    function compactEditorToolbar() {
        var toolbar = document.querySelector('.cod-canvas-toolbar');
        ...
        function menu(labelText, ids) {
            ...
            details.className = 'cod-toolbar-menu';
            var summary = document.createElement('summary');
            summary.className = 'button';
            summary.textContent = labelText;
            var panel = document.createElement('div');
            panel.className = 'cod-toolbar-menu__panel';
            ids.forEach(function (id) { moveWithLabel(id, panel); });
            details.append(summary, panel);
            details.addEventListener('toggle', function () {
                if (!details.open) return;
                toolbar.querySelectorAll('.cod-toolbar-menu[open]').forEach(function (other) {
                    if (other !== details) other.open = false;
                });
            });
            toolbar.insertBefore(details, document.getElementById('cod-canvas-publish'));
        }
        var publishButton = document.getElementById('cod-canvas-publish');
        var pagePicker = document.querySelector('.cod-canvas-page-picker');
        ['cod-page-target', 'cod-page-create', 'cod-snapshots-open', 'cod-canvas-page-title', 'cod-canvas-page-status'].forEach(function (id) {
        ...
        var historyButton = document.getElementById('cod-snapshots-open');
        ...
        menu('Vista', ['cod-canvas-zoom', 'cod-canvas-width', 'cod-canvas-height']);
        menu('Archivo', ['cod-canvas-reload', 'cod-canvas-export-html', 'cod-canvas-export-css', 'cod-canvas-toggle-import']);
    }
    ...
    var core = window.OCDEditorCore && typeof window.OCDEditorCore.create === 'function'
        ? window.OCDEditorCore.create({
            container: '#cod-canvas-editor-root',
            status: setStatus,
    ...
    // Expuesto para que cod-computed-inspector.js (control universal "Fuente
    ...
    // ------------------------------------------------------------------
    var zoomSelect = document.getElementById('cod-canvas-zoom');
    var widthSelect = document.getElementById('cod-canvas-width');
    var heightInput = document.getElementById('cod-canvas-height');
    var canvasRoot = document.getElementById('cod-canvas-editor-root');
    ...
    // del inspector (cod-computed-inspector.js) pueda guardar uno nuevo sin
    ...
    window.OCDCanvasEditor.saveCustomModule = function (component, label, category) {
        ...
        if (typeof component.addAttributes === 'function') {
            component.addAttributes({ 'data-cod-supermodule': '1' });
        }
     ...
     * ocultos #cod-region-targets / #cod-region-excludes se sincronizan desde
     ...
     * COD_Canvas_Document_Repository::sanitize_match_rules(). Solo es UI.
     */
    var RULE_TYPE_DEFS = {
    ...
    function updateMeta(doc) {
        var revision = document.getElementById('cod-canvas-revision');
        var updated = document.getElementById('cod-canvas-updated');
        if (revision) {
    ...
    function regionStatus(message, kind) {
        var node = document.getElementById('cod-canvas-region-status');
        if (!node) {
            return;
        }
        node.textContent = message;
        node.className = 'cod-canvas-status' + (kind ? ' is-' + kind : '');
    }
    ...
    function renderRuleChips(listName) {
        var container = document.querySelector('[data-cod-rule-list="' + listName + '"]');
        if (!container) {
            return;
        }
        container.innerHTML = '';
        ...
        rules.forEach(function (rule) {
            ...
            label.className = 'cod-rule-chip__label';
            label.textContent = ruleLabel(rule);
            chip.appendChild(label);

            var remove = document.createElement('button');
            remove.type = 'button';
            remove.className = 'cod-rule-chip__remove';
    ...
    function populateEntitySelect(listName, entityKind) {
        var entitySelect = document.querySelector('[data-cod-rule-entity="' + listName + '"]');
        if (!entitySelect) {
            return;
        }
    ...
    function updateRulePicker(listName) {
        var typeSelect = document.querySelector('[data-cod-rule-type="' + listName + '"]');
        var entitySelect = document.querySelector('[data-cod-rule-entity="' + listName + '"]');
        if (!typeSelect) {
            return;
        }
    ...
    function updateRuleFieldsetState() {
        var scopeField = document.getElementById('cod-region-scope');
        var scope = scopeField ? scopeField.value : '';
        var targetsFieldset = document.querySelector('[data-cod-rule-fieldset="targets"]');
        if (!targetsFieldset) {
            return;
        }
        var isGlobal = scope === 'global';
        targetsFieldset.classList.toggle('is-scoped-out', isGlobal);
        var hint = targetsFieldset.querySelector('[data-cod-rule-scope-hint="targets"]');
        if (hint) {
    ...
    function updateRegionReach() {
        var node = document.getElementById('cod-region-reach');
        if (!node) {
            return;
        }
        var scopeField = document.getElementById('cod-region-scope');
    ...
    function updateAdvancedJson() {
        var area = document.getElementById('cod-region-json');
        if (!area) {
            return;
        }
    ...
    function syncRegionState() {
        var targetsHidden = document.getElementById('cod-region-targets');
        var excludesHidden = document.getElementById('cod-region-excludes');
        if (targetsHidden) {
    ...
    function populateRegionFields(doc) {
        var kindField = document.getElementById('cod-region-kind');
        var scopeField = document.getElementById('cod-region-scope');
        if (!doc) {
            doc = {};
        }
    ...
    function saveRegion() {
        var kindField = document.getElementById('cod-region-kind');
        var scopeField = document.getElementById('cod-region-scope');
        var targetsField = document.getElementById('cod-region-targets');
        var excludesField = document.getElementById('cod-region-excludes');
    ...
    function exportHtml() {
        ...
        var page =
            ...
            '<title>ContOpe Canvas</title>\n' +
            '<style>\n' + getSourceCss() + '\n' + exported.css + '\n</style>\n</head>\n' +
            bodyMarkup +
            '\n<script>\n' + exported.js + '\n<\/script>\n</html>\n';
        download('contope-canvas.html', 'text/html;charset=utf-8', page);
    ...
    function applyImport() {
        var htmlInput = document.getElementById('cod-canvas-import-html');
        var cssInput = document.getElementById('cod-canvas-import-css');
    ...
    function updatePublishedPage(page) {
        var link = document.getElementById('cod-canvas-view-page');
        var title = document.getElementById('cod-canvas-page-title');
    ...
    function acfFieldBlockContent(name, type) {
        if (type === 'image') {
            return (
                '<img class="cod-dynamic-placeholder cod-dynamic-acf-image" ' +
                'data-cod-dynamic="acf_image:' + name + '" src="' + acfImageSrc + '" alt="">'
            );
        }
        ...
        return (
            '<' + tag + ' class="cod-dynamic-placeholder cod-dynamic-acf-field" ' +
            'data-cod-dynamic="acf:' + name + htmlSuffix + '">{{acf:' + name + htmlSuffix + '}}</' + tag + '>'
        );
    }
    ...
    function renderAcfFieldBlocks(fields) {
        ...
        // inspector (cod-computed-inspector.js) los pueda leer. El evento cubre
    ...
    function publishPage() {
        ...
        var title = document.getElementById('cod-canvas-page-title');
        var button = document.getElementById('cod-canvas-publish');
        var previewWindow = null;
        try {
            previewWindow = window.open('', 'cod-canvas-published-page');
            if (previewWindow) {
                previewWindow.document.title = 'Publicando ContOpe Canvas…';
    ...
    function activateSidePanel(name) {
        var workspace = document.querySelector('.cod-canvas-workspace');
        var tabs = document.querySelectorAll('[data-cod-side-panel]');
        if (!workspace || (name !== 'components' && name !== 'inspector')) return;
        workspace.setAttribute('data-cod-active-panel', name);
        Array.prototype.forEach.call(tabs, function (tab) {
            var active = tab.getAttribute('data-cod-side-panel') === name;
    ...
    function pageStatus(message, kind) {
        var node = document.getElementById('cod-canvas-page-status');
        if (!node) {
            return;
        }
        node.textContent = message;
    ...
    function getSlotElement(kind) {
        return document.querySelector('[data-cod-slot="' + kind + '"]');
    }

    function getSlotPreviewElement(kind) {
        return document.querySelector('[data-cod-slot-preview="' + kind + '"]');
    }

    function getSlotCanvasElement(kind) {
        return document.querySelector('[data-cod-slot-canvas="' + kind + '"]');
    }
    ...
    function renderSlotPreview(kind, doc) {
        ...
        content.className = 'cod-region-preview-content';
    ...
    function moveShellInto(kind) {
        var shell = document.getElementById('cod-canvas-editor-shell');
    ...
    function closeSnapshotsPanel() {
        var existing = document.getElementById('cod-snapshots-panel');
     ...
     * disparaba desde cod-inline-editor.js (edición en línea sobre la
    ...
    function createNewPage() {
        ...
        request(config.createPageAction, { title: title })
            .then(function (result) {
                var select = document.getElementById('cod-page-target');
                if (select) {
                    var option = document.createElement('option');
    ...
    function loadTargetPage(pageIdOverride) {
        ...
        var input = document.getElementById('cod-page-target');
    ...
    on('cod-canvas-save', save);
    on('cod-canvas-reload', function () {
        if (window.confirm('Recargar descarta los cambios no guardados del lienzo. ¿Continuar?')) {
            reload();
        }
    });
    on('cod-canvas-export-html', exportHtml);
    on('cod-canvas-export-css', exportCss);
    on('cod-canvas-import-apply', applyImport);
    ...
    document.querySelectorAll('[data-cod-side-panel]').forEach(function (tab) {
        tab.addEventListener('click', function () {
            activateSidePanel(tab.getAttribute('data-cod-side-panel'));
        });
    });
    on('cod-page-load', loadTargetPage);
    on('cod-page-create', createNewPage);
    on('cod-snapshots-open', openSnapshotsPanel);
    document.querySelectorAll('[data-cod-slot-preview]').forEach(function (preview) {
        var kind = preview.getAttribute('data-cod-slot-preview');
    ...
    var pageTarget = document.getElementById('cod-page-target');
    if (pageTarget) {
    ...
    Array.prototype.forEach.call(
        document.querySelectorAll('[data-cod-rule-type]'),
        function (select) {
            var listName = select.getAttribute('data-cod-rule-type');
    ...
    Array.prototype.forEach.call(
        document.querySelectorAll('[data-cod-rule-add]'),
        function (button) {
            var listName = button.getAttribute('data-cod-rule-add');
            button.addEventListener('click', function () {
                addRuleFromPicker(listName);
            });
        }
    );
    var regionScopeSelect = document.getElementById('cod-region-scope');
    ...
    var advancedJsonArea = document.getElementById('cod-region-json');
    if (advancedJsonArea) {
        advancedJsonArea.addEventListener('blur', onAdvancedJsonBlur);
    }

    on('cod-canvas-toggle-import', function (event) {
        var panel = document.getElementById('cod-canvas-import');
        if (!panel) {
            return;
        }
    ...
    readFileInto(
        document.getElementById('cod-canvas-import-html-file'),
        document.getElementById('cod-canvas-import-html')
    );
    readFileInto(
        document.getElementById('cod-canvas-import-css-file'),
        document.getElementById('cod-canvas-import-css')
    );
    ...
    var pageTitleInput = document.getElementById('cod-canvas-page-title');
    if (pageTitleInput && config.pageTitle) {
        pageTitleInput.value = config.pageTitle;
    }
    // Abrir siempre con los controles ContOpe Design visibles. La pestaña de
    ...
    if (config.autoLoadPageId && config.autoLoadPageId > 0) {
        var autoTarget = document.getElementById('cod-page-target');
        if (autoTarget) {
