/**
 * ContOpe Publisher — pantalla Plantillas del tema.
 *
...
(function (window, document) {
    ...
    function setStatus(container, message, kind) {
        ...
        var status = container.querySelector('.cod-tb-status');
        if (!status) {
            return;
        }
    ...
    function cardOf(node) {
        return node ? node.closest('.cod-tb-card') : null;
    }

    function regionOf(node) {
        return node ? node.closest('.cod-tb-region') : null;
    }
    ...
    function cardNameOf(card) {
        var name = card ? card.querySelector('.cod-tb-card-name') : null;
    ...
    function openRegion(region, isOpen) {
        ...
        var panel = region.querySelector('.cod-tb-region-panel');
        var chevron = region.querySelector('.cod-tb-chevron');
    ...
    function openKindsOf(card) {
        ...
        card.querySelectorAll('.cod-tb-region.is-open').forEach(function (region) {
    ...
    function replaceCard(cardHtml, oldCard) {
        ...
        openKinds.forEach(function (kind) {
            openRegion(fresh.querySelector('.cod-tb-region[data-region-kind="' + kind + '"]'), true);
        });
        return fresh;
    }
    ...
    function closeMenus(scope) {
        ...
        root.querySelectorAll('.cod-tb-menu.is-open').forEach(function (menu) {
            menu.classList.remove('is-open');
            var panel = menu.querySelector('.cod-tb-menu-panel');
            if (panel) {
                panel.hidden = true;
            }
        });
    ...
    function refreshScopeForm(form) {
        ...
        var scope = form.querySelector('.cod-tb-scope');
        var targets = form.querySelector('.cod-tb-targets');
        if (!scope || !targets) {
            return;
        }
    ...
    document.addEventListener('click', function (event) {
        ...
        var newButton = target.closest('#cod-tb-new-template');
        if (newButton) {
            ...
            request(config.createTemplateAction, { title: title })
                .then(function (data) {
                    var grid = document.getElementById('cod-tb-grid');
        ...
        var chevron = target.closest('.cod-tb-chevron');
        if (chevron) {
            event.preventDefault();
        ...
        var fork = target.closest('.cod-tb-fork');
        if (fork) {
            event.preventDefault();
        ...
        var menuToggle = target.closest('.cod-tb-menu-toggle');
        if (menuToggle) {
            event.preventDefault();
            var menu = menuToggle.closest('.cod-tb-menu');
            if (!menu) {
                return;
            }
            ...
            if (willOpen) {
                ...
                var menuPanel = menu.querySelector('.cod-tb-menu-panel');
                if (menuPanel) {
        ...
        var menuRename = target.closest('.cod-tb-menu-rename');
        if (menuRename) {
            event.preventDefault();
            var renameCard = cardOf(menuRename);
            closeMenus(document);
            var renameForm = renameCard ? renameCard.querySelector('.cod-tb-rename') : null;
            if (renameForm) {
        ...
        var menuDelete = target.closest('.cod-tb-menu-delete');
        if (menuDelete) {
            event.preventDefault();
        ...
        var renameCancel = target.closest('.cod-tb-rename-cancel');
        if (renameCancel) {
            event.preventDefault();
            var cancelForm = renameCancel.closest('.cod-tb-rename');
            if (cancelForm) {
    ...
    document.addEventListener('change', function (event) {
        ...
        if (select) {
            ...
            request(config.assignRegionAction, {
                ...
                .then(function (data) {
                    ...
                    var statusRegion = fresh.querySelector('.cod-tb-region[data-region-kind="' + kind + '"]');
        ...
        var scope = target.closest('.cod-tb-scope');
        if (scope) {
            refreshScopeForm(scope.closest('.cod-tb-scope-form'));
        }
    });
    ...
    document.addEventListener('submit', function (event) {
        ...
        if (form.classList.contains('cod-tb-scope-form')) {
            event.preventDefault();
            ...
            var scopeSelect = form.querySelector('.cod-tb-scope');
            var targetsSelect = form.querySelector('.cod-tb-targets');
            var excludesSelect = form.querySelector('.cod-tb-excludes');
            var summary = form.querySelector('.cod-tb-scope-summary');
        ...
        if (form.classList.contains('cod-tb-rename')) {
            event.preventDefault();
        ...
        if (form.classList.contains('cod-tb-region-name-form')) {
            event.preventDefault();
