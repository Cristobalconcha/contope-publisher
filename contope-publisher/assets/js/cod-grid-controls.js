(function installOcdGridControls(global) {
  ...
  function installStyles(document) {
    if (document.getElementById('cod-grid-controls-css')) return;
    const style = document.createElement('style');
    style.id = 'cod-grid-controls-css';
    style.textContent = `
      .cod-gc { margin-top:12px; padding-top:12px; border-top:1px solid #ffffff18; }
      .cod-gc__title { display:flex; justify-content:space-between; gap:8px; margin-bottom:7px; font-weight:650; }
      .cod-gc__target { color:#c7bda9; font-weight:400; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
      .cod-gc__row { display:grid; grid-template-columns:1fr 1fr 1fr; gap:7px; margin-top:7px; }
      .cod-gc label { display:grid; gap:4px; color:#aaa49b; font-size:10px; }
      .cod-gc select,.cod-gc input,.cod-gc button { width:100%; min-width:0; border:1px solid #ffffff24; border-radius:5px;
        background:#2a2e36; color:#fff; padding:6px 7px; font:inherit; }
      .cod-gc button { cursor:pointer; background:#a7641a; border-color:#b8752a; font-weight:650; }
      .cod-gc button:disabled,.cod-gc input:disabled { opacity:.42; cursor:not-allowed; }
      .cod-gc__breakpoints { display:flex; align-items:center; gap:3px; margin-left:auto; }
      .cod-gc__breakpoint { width:34px !important; height:27px; padding:3px !important; border-color:#ffffff24 !important; background:#252930 !important; color:#aeb4bd !important; }
      .cod-gc__breakpoint svg { display:block; width:100%; height:100%; }
      .cod-gc__breakpoint.is-active { border-color:#72aee6 !important; background:#315b83 !important; color:#fff !important; }
    `;
  ...
  function create(editor, gridApi, options) {
    ...
    const mount = opts.mount || hostDocument.querySelector('.cod-ci__head') || hostDocument.body;
    installStyles(hostDocument);

    const root = element(hostDocument, 'section', 'cod-gc');
    const title = element(hostDocument, 'div', 'cod-gc__title');
    title.append(element(hostDocument, 'span', '', 'Columnas'), element(hostDocument, 'span', 'cod-gc__target', 'sin contenedor'));
    const row = element(hostDocument, 'div', 'cod-gc__row');
    ...
    const breakpointBar = element(hostDocument, 'div', 'cod-gc__breakpoints');
    ...
    for (const [value, label] of [['desktop', 'Escritorio'], ['tablet', 'Tablet'], ['mobile', 'Móvil']]) {
      const button = element(hostDocument, 'button', 'cod-gc__breakpoint');
    ...
    (mount.querySelector('.cod-ci__headbar') || mount).appendChild(breakpointBar);
    ...
    function refresh(component) {
      ...
      root.querySelector('.cod-gc__target').textContent = describe(selectedGrid);
