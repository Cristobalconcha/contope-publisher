/* Generated from cod-canvas-grid.js; run npm run build:grid-global. */
/**
 * ContOpe Canvas Grid for GrapesJS.
 *
...
const GRID_ATTRIBUTE = 'data-cod-grid-id';
...
function getGridId(component) {
  ...
  return `cod-grid-${sanitizeIdentifier(source)}`;
}
...
function addGridStyleSector(editor) {
  ...
  editor.StyleManager.addType('cod-grid-template', {
    create({ change }) {
    ...
    update({ value, el }) {
      const select = el.querySelector('[data-cod-grid-preset]');
      const input = el.querySelector('[data-cod-grid-custom]');
  ...
  const gridSector = editor.StyleManager.getSector('cod-canvas-grid');
...
function createCanvasGridApi(editor, options = {}) {
  ...
  // (ej. ".cod-columns.cod-columns--single.cod-grid-<id>", especificidad
  // 0-3-0) que le ganan a la regla canónica de una sola clase
  // ".cod-grid-<id>" (especificidad 0-1-0) que este motor usa siempre para
  ...
  function applyTemplate(component, template, settings = {}) {
    ...
    // clase .cod-grid-<id> que usa este motor, asi que la regla de acá
  ...
  return {
    ...
    // en cod-computed-inspector.js) lean el mismo breakpoint activo que ya
...
function ocdCanvasGrid(editor, options = {}) {
  ...
  editor.Commands?.add?.('cod-grid:scan', { run: () => api.scan() });
  editor.Commands?.add?.('cod-grid:apply-preset', {
