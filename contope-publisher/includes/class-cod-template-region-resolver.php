 ...
 */
final class COD_Template_Region_Resolver
{
    /** Highest number wins when multiple targets on the same document match. */
    private const SPECIFICITY = [
        COD_Canvas_Document_Repository::MATCH_TYPE_POST => 5,
        COD_Canvas_Document_Repository::MATCH_TYPE_CHILDREN_OF => 4,
        COD_Canvas_Document_Repository::MATCH_TYPE_CATEGORY => 3,
        COD_Canvas_Document_Repository::MATCH_TYPE_TAG => 3,
        COD_Canvas_Document_Repository::MATCH_TYPE_HOMEPAGE => 2,
        COD_Canvas_Document_Repository::MATCH_TYPE_ALL_PAGES => 1,
        COD_Canvas_Document_Repository::MATCH_TYPE_ALL_POSTS => 1,
    ];
    ...
    public function resolve(string $kind, int $post_id)
    {
        ...
        foreach ($candidates as $doc) {
            if ($doc['regionScope'] === COD_Canvas_Document_Repository::REGION_SCOPE_GLOBAL) {
            ...
            if ($doc['regionScope'] !== COD_Canvas_Document_Repository::REGION_SCOPE_LOCAL) {
                continue;
            }
            if ($this->any_rule_matches($doc['regionExcludes'] ?? [], $context)) {
                continue;
