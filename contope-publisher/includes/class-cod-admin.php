...
final class COD_Admin
{
    private const ACTION = 'cod_import_project';

    public const MENU_SLUG = 'contope-publisher';
    public const CAPABILITY = 'manage_options';

    public function __construct(private COD_Importer $importer)
    {
    }

    public function register(): void
    {
        // Sin entrada de menú propia: COD_Site_Package_Admin (Portabilidad)
        // es ahora la única pantalla del menú "ContOpe Design" y cubre este
    ...
    public function render_page(): void
    {
        if (!current_user_can(self::CAPABILITY)) {
            wp_die(esc_html__('No tienes permisos para importar proyectos.', 'contope-publisher'));
        }
        $status = isset($_GET['cod_status']) ? sanitize_key(wp_unslash($_GET['cod_status'])) : '';
        ?>
        <div class="wrap">
            <h1>ContOpe Publisher</h1>
            ...
            <form method="post" enctype="multipart/form-data" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <input type="file" name="cod_project" accept="application/json,.json" required>
    ...
    public function handle_import(): void
    {
        ...
        wp_safe_redirect(add_query_arg(['page' => self::MENU_SLUG, 'cod_status' => 'success'], admin_url('admin.php')));
        exit;
    }
    ...
    {
        return 'cod_import_error_' . get_current_user_id();
    }
}
