...
final class COD_Importer
{
    public function __construct(
        private COD_Package_Validator $validator,
        private COD_Block_Serializer $serializer
    ) {
    }
    ...
    public function import_json(string $json)
    {
        if (strlen($json) > 2 * MB_IN_BYTES) {
            return new WP_Error('cod_file_size', 'El documento supera el límite de 2 MB.');
        }

        try {
            $document = json_decode($json, true, 128, JSON_THROW_ON_ERROR);
        } catch (JsonException $exception) {
            return new WP_Error('cod_json', 'El documento JSON no es válido.');
        }
        if (!is_array($document)) {
            return new WP_Error('cod_document', 'El documento debe ser un objeto JSON.');
        }
        ...
        foreach ($document['pages'] as $page) {
            $post_id = wp_insert_post(
                [
                    ...
                    'meta_input' => [
                        '_cod_project_id' => sanitize_key($document['project']['id']),
                        '_cod_page_id' => sanitize_key($page['id']),
                        '_cod_schema_version' => 0,
                    ],
                ],
                true
            );
