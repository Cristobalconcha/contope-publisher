 ...
 */
final class OCD_Canvas_MCP_Recipe_Compiler
{
    ...
    public function __construct(private OCD_Canvas_Document_Sanitizer $sanitizer)
    {
    }
    ...
    public function capability_catalog(): array
    {
        return [
            ...
            'composition' => [
                ...
                'nodeRecord' => [
                    'required' => ['id', 'kind'],
                    'optional' => ['ruleIds', 'cadenceRuleId', 'children', 'content'],
            ...
            'realization' => [
                ...
                'assetPolicy' => 'ocd_resolve_canvas_assets devuelve activos ya gestionados por Canvas. El compilador acepta URLs seguras, no carga archivos ni verifica recursos remotos.',
                'formPolicy' => 'Los formularios se eligen desde ocd_list_canvas_forms; Canvas no recibe HTML de formulario remoto.',
            ],
    ...
    private function rule_value_schemas(): array
    {
        return [
            ...
            'spacing' => [
                'atLeastOneOf' => ['paddingBlock', 'paddingInline', 'marginBlockStart', 'marginBlockEnd', 'gap', 'indent', 'bleed'],
                'notes' => 'bleed admite valor negativo; las demás medidas son seguras y positivas.',
            ],
            'layout' => [
                'required' => ['mode'],
                'fields' => [
            ...
            'surface' => [
                ...
                'fields' => [
                    'backgroundAssetUrl' => 'URL HTTP(S) o raíz relativa segura; usa ocd_resolve_canvas_assets para material existente',
    ...
    private function node_content_schemas(): array
    {
        return [
            ...
            'video' => ['content' => [
                ...
                'posterUrl' => 'opcional',
                'caption' => 'opcional',
                'matte' => 'opcional, boolean, default false. true = video con transparencia real (compositor ocd-luma-matte): el archivo fuente debe traer el video RGB arriba y su máscara blanco/negro abajo (mismo ancho, el doble de alto; blanco = visible, negro = transparente). Se renderiza como video oculto + canvas compuesto en vivo; ignora posterUrl.',
            ]],
            ...
            'form' => ['content' => ['formSlug' => 'valor devuelto por ocd_list_canvas_forms']],
            'shortcode' => ['content' => [
    ...
    public function render_whatsapp_module(string $node_id, string $message, string $position, array $style = [])
    {
        if (!$this->is_stable_id($node_id)) {
            return new WP_Error('ocd_mcp_whatsapp_invalid', 'node_id no es un id estable válido.');
        }
        ...
        $attrs = 'class="ocd-node ocd-node--whatsapp ocd-node-id-' . esc_attr($node_id) . '" data-ocd-node="' . esc_attr($node_id) . '"'
            . ' data-ocd-behavior="anchor" data-ocd-anchor-reveal-transform="' . esc_attr($anchor_style['revealedTransform']) . '"'
            . ' data-ocd-anchor-repeat="' . ($anchor_value['repeat'] ? '1' : '0') . '"'
    ...
    public function compile(array $composition, array $design)
    {
        ...
        if (strpos($rendered['markup'], '--ocd-motion-delay:') !== false && strpos($markup, '--ocd-motion-delay:') === false) {
            return new WP_Error('ocd_mcp_motion_sanitized_away', 'El saneador Canvas no preservó el stagger de movimiento; la composición no se guardó parcialmente.');
        }
        ...
        $project = [
            'pages' => [[
                'name' => 'Open CoDesign MCP composition',
                'component' => [
                    'type' => 'wrapper',
                    'components' => $markup,
                ],
        ...
        if (!is_string($project_json)) {
            return new WP_Error('ocd_mcp_composition_project_encode', 'No se pudo codificar el proyecto Canvas compuesto.');
        }
        ...
        return [
            ...
            // hay que volver a mandar tal cual a ocd_preview_canvas_composition /
            // ocd_apply_canvas_composition para editar un nodo puntual sin
            // reconstruir el resto a ciegas. Ver OCD_Canvas_Document_Repository::
    ...
    private function normalize_design(array $design)
    {
        ...
        if (!$this->has_only_keys($design, $allowed)
            ...
            return new WP_Error('ocd_mcp_design_rule_set_invalid', 'El conjunto de reglas de diseño no tiene la forma permitida.');
        }

        if (isset($design['label']) && (!is_string($design['label']) || mb_strlen($design['label']) > 160)) {
            return new WP_Error('ocd_mcp_design_rule_set_invalid', 'label del conjunto de reglas no es válido.');
        }
        ...
        foreach ($design['rules'] as $raw_rule) {
            if (!is_array($raw_rule)) {
                return new WP_Error('ocd_mcp_design_rule_invalid', 'Cada regla de diseño debe ser un objeto.');
            }
            ...
            if (isset($rule_index[$rule['id']])) {
                return new WP_Error('ocd_mcp_design_rule_duplicate', 'Los identificadores de reglas de diseño deben ser únicos.');
            }
        ...
        foreach ($rules as $rule) {
            ...
            foreach ($rule['value']['cycleRuleIds'] as $cycle_rule_id) {
                if (!isset($rule_index[$cycle_rule_id]) || $rule_index[$cycle_rule_id]['kind'] === 'cadence') {
                    return new WP_Error('ocd_mcp_cadence_rule_invalid', 'Una regla de cadencia sólo puede referir reglas existentes no-cadencia.');
                }
            }
        }
        ...
        if (!is_array($root_rule_ids) || !$this->is_list($root_rule_ids) || count($root_rule_ids) > 32) {
            return new WP_Error('ocd_mcp_design_rule_set_invalid', 'rootRuleIds debe ser una lista acotada.');
        }
        foreach ($root_rule_ids as $rule_id) {
            if (!is_string($rule_id) || !isset($rule_index[$rule_id])) {
                return new WP_Error('ocd_mcp_design_rule_reference_invalid', 'rootRuleIds contiene una regla inexistente.');
            }
        }
    ...
    private function normalize_rule(array $rule)
    {
        if (!$this->has_only_keys($rule, ['id', 'label', 'kind', 'scope', 'provenance', 'status', 'value'])
            ...
            return new WP_Error('ocd_mcp_design_rule_invalid', 'Una regla de diseño contiene campos no permitidos o incompletos.');
        }
    ...
    private function normalize_scope(array $scope)
    {
        if (!$this->has_only_keys($scope, ['breakpoint', 'state', 'roles'])) {
            return new WP_Error('ocd_mcp_design_scope_invalid', 'scope contiene un campo no permitido.');
        }
        ...
        if (!is_string($breakpoint) || !in_array($breakpoint, ['all', 'desktop', 'tablet', 'mobile'], true)
            ...
            return new WP_Error('ocd_mcp_design_scope_invalid', 'scope debe declarar breakpoint, state y roles válidos.');
        }
        foreach ($roles as $role) {
            if (!is_string($role) || !$this->is_stable_id($role)) {
                return new WP_Error('ocd_mcp_design_scope_invalid', 'scope.roles contiene un rol no válido.');
            }
        }
        return [
    ...
    private function normalize_provenance(array $provenance)
    {
        if (!$this->has_only_keys($provenance, ['sources', 'confidence'])
            ...
            return new WP_Error('ocd_mcp_design_provenance_invalid', 'Toda regla debe declarar al menos una fuente de procedencia.');
        }
        ...
        foreach ($provenance['sources'] as $source) {
            if (!is_array($source)
                ...
                return new WP_Error('ocd_mcp_design_provenance_invalid', 'Una fuente de procedencia no tiene la forma permitida.');
            }
            $sources[] = [
                'kind' => $source['kind'],
        ...
        if ($confidence !== null && (!is_float($confidence) && !is_int($confidence) || $confidence < 0 || $confidence > 1)) {
            return new WP_Error('ocd_mcp_design_provenance_invalid', 'confidence debe estar entre 0 y 1.');
        }
    ...
    private function normalize_rule_value(string $kind, array $value)
    {
        ...
        return new WP_Error('ocd_mcp_design_rule_invalid', 'El tipo de regla no está disponible.');
    }
    ...
    private function normalize_color_rule(array $value)
    {
        if (!$this->has_only_keys($value, ['role', 'color'])
            ...
            return new WP_Error('ocd_mcp_color_rule_invalid', 'La regla color requiere role y color seguros.');
        }
    ...
    private function normalize_typography_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || !isset($value['role']) || !$this->is_stable_id($value['role'])) {
            return new WP_Error('ocd_mcp_typography_rule_invalid', 'La regla typography requiere un role editorial.');
        }
        $normalized = ['role' => $value['role']];
        if (isset($value['family'])) {
            if (!is_string($value['family']) || !$this->is_css_font_family($value['family'])) {
                return new WP_Error('ocd_mcp_typography_rule_invalid', 'family tipográfica no es segura.');
            }
        ...
        foreach (['fontSize', 'letterSpacing', 'measure'] as $key) {
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !$this->is_css_length($value[$key], $key === 'letterSpacing')) {
                    return new WP_Error('ocd_mcp_typography_rule_invalid', $key . ' debe ser una longitud CSS segura.');
                }
        ...
        if (isset($value['fontWeight'])) {
            if (!is_int($value['fontWeight']) || $value['fontWeight'] < 100 || $value['fontWeight'] > 900 || $value['fontWeight'] % 100 !== 0) {
                return new WP_Error('ocd_mcp_typography_rule_invalid', 'fontWeight debe estar entre 100 y 900 en pasos de 100.');
            }
        ...
        if (isset($value['lineHeight'])) {
            if ((!is_int($value['lineHeight']) && !is_float($value['lineHeight'])) || $value['lineHeight'] < 0.8 || $value['lineHeight'] > 3) {
                return new WP_Error('ocd_mcp_typography_rule_invalid', 'lineHeight debe estar entre 0.8 y 3.');
            }
        ...
        foreach ([
            ...
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !in_array($value[$key], $allowed_values, true)) {
                    return new WP_Error('ocd_mcp_typography_rule_invalid', $key . ' no es un tratamiento editorial permitido.');
                }
                $normalized[$key] = $value[$key];
            }
        }
    ...
    {
        $allowed = ['paddingBlock', 'paddingInline', 'marginBlockStart', 'marginBlockEnd', 'gap', 'indent', 'bleed'];
        if (!$this->has_only_keys($value, $allowed) || $value === []) {
            return new WP_Error('ocd_mcp_spacing_rule_invalid', 'La regla spacing debe declarar al menos una medida.');
        }
        $normalized = [];
        foreach ($allowed as $key) {
            ...
            if (!is_string($value[$key]) || !$this->is_css_length($value[$key], $key === 'bleed')) {
                return new WP_Error('ocd_mcp_spacing_rule_invalid', $key . ' debe ser una longitud CSS segura.');
            }
            $normalized[$key] = $value[$key];
        }
        return $normalized;
    ...
    private function normalize_layout_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || !isset($value['mode'])
            ...
            return new WP_Error('ocd_mcp_layout_rule_invalid', 'layout.mode no es una composición disponible.');
        }
        $normalized = ['mode' => $value['mode']];
        foreach (['columns'] as $key) {
            if (isset($value[$key])) {
                if (!is_int($value[$key]) || $value[$key] < 1 || $value[$key] > 8) {
                    return new WP_Error('ocd_mcp_layout_rule_invalid', $key . ' debe estar entre 1 y 8.');
                }
                $normalized[$key] = $value[$key];
            }
        }
        foreach (['gap', 'minColumnWidth', 'maxWidth'] as $key) {
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !$this->is_css_length($value[$key])) {
                    return new WP_Error('ocd_mcp_layout_rule_invalid', $key . ' debe ser una longitud CSS segura.');
                }
                $normalized[$key] = $value[$key];
            }
        }
        foreach ([
            ...
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !in_array($value[$key], $allowed_values, true)) {
                    return new WP_Error('ocd_mcp_layout_rule_invalid', $key . ' no es válido.');
                }
                $normalized[$key] = $value[$key];
            }
        }
        if (isset($value['mobile'])) {
            if (!is_array($value['mobile']) || !$this->has_only_keys($value['mobile'], ['mode', 'columns', 'gap'])
                ...
                return new WP_Error('ocd_mcp_layout_rule_invalid', 'layout.mobile no tiene la forma permitida.');
            }
    ...
    private function normalize_surface_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || $value === []) {
            return new WP_Error('ocd_mcp_surface_rule_invalid', 'La regla surface debe declarar al menos un tratamiento.');
        }
        $normalized = [];
        foreach (['backgroundColor', 'foregroundColor', 'overlayColor', 'borderColor'] as $key) {
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !$this->is_css_color($value[$key])) {
                    return new WP_Error('ocd_mcp_surface_rule_invalid', $key . ' no es un color seguro.');
                }
                $normalized[$key] = $value[$key];
            }
        }
        if (isset($value['backgroundAssetUrl'])) {
            if (!is_string($value['backgroundAssetUrl']) || !$this->is_safe_asset_url($value['backgroundAssetUrl'])) {
                return new WP_Error('ocd_mcp_surface_rule_invalid', 'backgroundAssetUrl debe provenir de la resolución de activos Canvas.');
            }
        ...
        if (isset($value['backgroundPosition'])) {
            if (!is_string($value['backgroundPosition']) || !$this->is_object_position($value['backgroundPosition'])) {
                return new WP_Error('ocd_mcp_surface_rule_invalid', 'backgroundPosition no es válido.');
            }
        ...
        if (isset($value['backgroundSize'])) {
            if (!is_string($value['backgroundSize']) || !in_array($value['backgroundSize'], ['cover', 'contain', 'auto'], true)) {
                return new WP_Error('ocd_mcp_surface_rule_invalid', 'backgroundSize no es válido.');
            }
        ...
        if (isset($value['overlayOpacity'])) {
            if ((!is_int($value['overlayOpacity']) && !is_float($value['overlayOpacity'])) || $value['overlayOpacity'] < 0 || $value['overlayOpacity'] > 1) {
                return new WP_Error('ocd_mcp_surface_rule_invalid', 'overlayOpacity debe estar entre 0 y 1.');
            }
        ...
        if (isset($value['borderWidth'])) {
            if (!is_string($value['borderWidth']) || !$this->is_css_length($value['borderWidth'])) {
                return new WP_Error('ocd_mcp_surface_rule_invalid', 'borderWidth debe ser una longitud CSS segura.');
            }
        ...
        if (isset($value['shadow'])) {
            if (!is_string($value['shadow']) || !in_array($value['shadow'], ['none', 'sm', 'md', 'lg'], true)) {
                return new WP_Error('ocd_mcp_surface_rule_invalid', 'shadow no es válido.');
            }
    ...
    private function normalize_shape_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || $value === []) {
            return new WP_Error('ocd_mcp_shape_rule_invalid', 'La regla shape debe declarar al menos un tratamiento.');
        }
        $normalized = [];
        if (isset($value['radius'])) {
            if (!is_string($value['radius']) || (!$this->is_css_length($value['radius']) && !in_array($value['radius'], ['none', 'pill', 'circle'], true))) {
                return new WP_Error('ocd_mcp_shape_rule_invalid', 'radius no es válido.');
            }
        ...
        if (isset($value['borderStyle'])) {
            if (!is_string($value['borderStyle']) || !in_array($value['borderStyle'], ['none', 'solid', 'dashed'], true)) {
                return new WP_Error('ocd_mcp_shape_rule_invalid', 'borderStyle no es válido.');
            }
            $normalized['borderStyle'] = $value['borderStyle'];
        }
        if (isset($value['borderWidth'])) {
            if (!is_string($value['borderWidth']) || !$this->is_css_length($value['borderWidth'])) {
                return new WP_Error('ocd_mcp_shape_rule_invalid', 'borderWidth no es válido.');
            }
            $normalized['borderWidth'] = $value['borderWidth'];
        }
        if (isset($value['mask'])) {
            if (!is_string($value['mask']) || !in_array($value['mask'], ['none', 'rounded', 'circle', 'arch'], true)) {
                return new WP_Error('ocd_mcp_shape_rule_invalid', 'mask no es válido.');
            }
    ...
    private function normalize_media_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || $value === []) {
            return new WP_Error('ocd_mcp_media_rule_invalid', 'La regla media debe declarar al menos un tratamiento.');
        }
        $normalized = [];
        if (isset($value['aspectRatio'])) {
            if (!is_string($value['aspectRatio']) || preg_match('/^[1-9][0-9]?[ ]*\/[ ]*[1-9][0-9]?$/', $value['aspectRatio']) !== 1) {
                return new WP_Error('ocd_mcp_media_rule_invalid', 'aspectRatio debe tener la forma 4/3, 16/9 u otra proporción segura.');
            }
        ...
        if (isset($value['fit'])) {
            if (!is_string($value['fit']) || !in_array($value['fit'], ['cover', 'contain', 'fill', 'none', 'scale-down'], true)) {
                return new WP_Error('ocd_mcp_media_rule_invalid', 'fit no es válido.');
            }
            $normalized['fit'] = $value['fit'];
        }
        if (isset($value['position'])) {
            if (!is_string($value['position']) || !$this->is_object_position($value['position'])) {
                return new WP_Error('ocd_mcp_media_rule_invalid', 'position no es válido.');
            }
        ...
        foreach ([
            ...
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !in_array($value[$key], $allowed_values, true)) {
                    return new WP_Error('ocd_mcp_media_rule_invalid', $key . ' no es válido.');
                }
                $normalized[$key] = $value[$key];
            }
        }
        if (isset($value['overlayColor'])) {
            if (!is_string($value['overlayColor']) || !$this->is_css_color($value['overlayColor'])) {
                return new WP_Error('ocd_mcp_media_rule_invalid', 'overlayColor no es válido.');
            }
            $normalized['overlayColor'] = $value['overlayColor'];
        }
        if (isset($value['overlayOpacity'])) {
            if ((!is_int($value['overlayOpacity']) && !is_float($value['overlayOpacity'])) || $value['overlayOpacity'] < 0 || $value['overlayOpacity'] > 1) {
                return new WP_Error('ocd_mcp_media_rule_invalid', 'overlayOpacity debe estar entre 0 y 1.');
            }
            $normalized['overlayOpacity'] = (float) $value['overlayOpacity'];
        }
        return $normalized;
    ...
    private function normalize_button_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || $value === []) {
            return new WP_Error('ocd_mcp_button_rule_invalid', 'La regla button debe declarar al menos un tratamiento.');
        }
        $normalized = [];
        foreach ([
            ...
            if (!is_string($value[$key]) || !in_array($value[$key], $allowed_values, true)) {
                return new WP_Error('ocd_mcp_button_rule_invalid', $key . ' no es válido.');
            }
            $normalized[$key] = $value[$key];
        }
        ...
        if (isset($value['zIndex'])) {
            if (!is_int($value['zIndex']) || $value['zIndex'] < -999 || $value['zIndex'] > 999) {
                return new WP_Error('ocd_mcp_button_rule_invalid', 'zIndex debe ser un entero entre -999 y 999.');
            }
    ...
    private function normalize_gallery_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || !isset($value['mode'])
            ...
            return new WP_Error('ocd_mcp_gallery_rule_invalid', 'gallery.mode debe ser grid, metro, masonry o carousel.');
        }
        $normalized = ['mode' => $value['mode']];
        foreach (['columns', 'mobileColumns'] as $key) {
            if (isset($value[$key])) {
                if (!is_int($value[$key]) || $value[$key] < 1 || $value[$key] > 8) {
                    return new WP_Error('ocd_mcp_gallery_rule_invalid', $key . ' debe estar entre 1 y 8.');
                }
                $normalized[$key] = $value[$key];
            }
        }
        if (isset($value['gap'])) {
            if (!is_string($value['gap']) || !$this->is_css_length($value['gap'])) {
                return new WP_Error('ocd_mcp_gallery_rule_invalid', 'gap debe ser una longitud CSS segura.');
            }
        ...
        foreach ([
            ...
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !in_array($value[$key], $allowed_values, true)) {
                    return new WP_Error('ocd_mcp_gallery_rule_invalid', $key . ' no es válido.');
                }
                $normalized[$key] = $value[$key];
            }
        }
    ...
    private function normalize_table_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || $value === []) {
            return new WP_Error('ocd_mcp_table_rule_invalid', 'La regla table debe declarar al menos un tratamiento.');
        }
        $normalized = [];
        foreach ([
            ...
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !in_array($value[$key], $allowed_values, true)) {
                    return new WP_Error('ocd_mcp_table_rule_invalid', $key . ' no es válido.');
                }
                $normalized[$key] = $value[$key];
            }
        }
    ...
    private function normalize_form_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || $value === []) {
            return new WP_Error('ocd_mcp_form_rule_invalid', 'La regla form debe declarar al menos un tratamiento.');
        }
        $normalized = [];
        ...
        if (isset($value['maxWidth'])) {
            if (!is_string($value['maxWidth']) || !$this->is_css_length($value['maxWidth'])) {
                return new WP_Error('ocd_mcp_form_rule_invalid', 'maxWidth debe ser una longitud CSS segura.');
            }
        ...
        foreach (['surface' => ['none', 'card', 'outlined']] as $key => $allowed_values) {
            if (isset($value[$key])) {
                if (!is_string($value[$key]) || !in_array($value[$key], $allowed_values, true)) {
                    return new WP_Error('ocd_mcp_form_rule_invalid', $key . ' no es válido.');
                }
                $normalized[$key] = $value[$key];
            }
        }
    ...
    private function normalize_motion_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || !isset($value['trigger'])
            ...
            return new WP_Error('ocd_mcp_motion_rule_invalid', 'motion.trigger debe ser load, scroll u hover.');
        }
        $normalized = [
        ...
        if (!is_string($normalized['effect']) || !in_array($normalized['effect'], ['fade', 'rise', 'slide-left', 'slide-right', 'scale'], true)
            ...
            return new WP_Error('ocd_mcp_motion_rule_invalid', 'La paleta de movimiento contiene un valor no permitido.');
        }
        if ($normalized['trigger'] === 'scroll') {
            ...
            if ((!is_int($threshold) && !is_float($threshold)) || $threshold < 0 || $threshold > 1) {
                return new WP_Error('ocd_mcp_motion_rule_invalid', 'threshold de scroll debe estar entre 0 y 1.');
            }
            $normalized['threshold'] = (float) $threshold;
        } elseif (isset($value['threshold'])) {
            return new WP_Error('ocd_mcp_motion_rule_invalid', 'threshold sólo puede usarse con motion.trigger=scroll.');
        }
        if ($normalized['trigger'] === 'load' && ($normalized['delay'] !== 0 || $normalized['stagger'] !== 0)) {
            return new WP_Error('ocd_mcp_motion_rule_invalid', 'El runtime load actual no admite delay ni stagger; usa motion.trigger=scroll para entrada escalonada.');
        }
        if ($normalized['trigger'] === 'hover' && $normalized['stagger'] !== 0) {
            return new WP_Error('ocd_mcp_motion_rule_invalid', 'motion.trigger=hover no admite stagger por ítem.');
        }
        return $normalized;
    }
    ...
    private function normalize_interaction_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed) || !isset($value['behavior'])
            ...
            return new WP_Error('ocd_mcp_interaction_rule_invalid', 'interaction.behavior no es un comportamiento Canvas disponible.');
        }
        ...
        if (isset($value['threshold'])) {
            if (!is_int($value['threshold']) || $value['threshold'] < 0 || $value['threshold'] > 4000) {
                return new WP_Error('ocd_mcp_interaction_rule_invalid', 'threshold debe estar entre 0 y 4000.');
            }
            $normalized['threshold'] = $value['threshold'];
        }
        if (isset($value['targetId'])) {
            if (!is_string($value['targetId']) || !$this->is_stable_id($value['targetId'])) {
                return new WP_Error('ocd_mcp_interaction_rule_invalid', 'targetId no es válido.');
            }
        ...
        if (isset($value['toggleClass'])) {
            if (!is_string($value['toggleClass']) || preg_match('/^[a-z][a-z0-9_-]{0,63}$/', $value['toggleClass']) !== 1) {
                return new WP_Error('ocd_mcp_interaction_rule_invalid', 'toggleClass no es una clase segura.');
            }
            $normalized['toggleClass'] = $value['toggleClass'];
        }
        if (isset($value['mode'])) {
            if (!is_string($value['mode']) || !in_array($value['mode'], ['single', 'track'], true)) {
                return new WP_Error('ocd_mcp_interaction_rule_invalid', 'mode sólo admite single o track.');
            }
            $normalized['mode'] = $value['mode'];
        }
        foreach (['visible', 'visibleMobile'] as $key) {
            if (isset($value[$key])) {
                if (!is_int($value[$key]) || $value[$key] < 1 || $value[$key] > 8) {
                    return new WP_Error('ocd_mcp_interaction_rule_invalid', $key . ' debe estar entre 1 y 8.');
                }
                $normalized[$key] = $value[$key];
            }
        }
        if ($normalized['behavior'] === 'nav-toggle' && !isset($normalized['targetId'])) {
            return new WP_Error('ocd_mcp_interaction_rule_invalid', 'nav-toggle requiere targetId.');
        }
        return $normalized;
    }
    ...
    private function normalize_cadence_rule(array $value)
    {
        if (!$this->has_only_keys($value, ['cycleRuleIds', 'offset'])
            ...
            return new WP_Error('ocd_mcp_cadence_rule_invalid', 'cadence requiere una lista acotada cycleRuleIds.');
        }
        $cycle_rule_ids = [];
        foreach ($value['cycleRuleIds'] as $rule_id) {
            if (!is_string($rule_id) || !$this->is_stable_id($rule_id) || in_array($rule_id, $cycle_rule_ids, true)) {
                return new WP_Error('ocd_mcp_cadence_rule_invalid', 'cycleRuleIds debe contener ids estables únicos.');
            }
            $cycle_rule_ids[] = $rule_id;
        }
        $offset = $value['offset'] ?? 0;
        if (!is_int($offset) || $offset < 0 || $offset > 11) {
            return new WP_Error('ocd_mcp_cadence_rule_invalid', 'offset de cadence debe estar entre 0 y 11.');
        }
    ...
    private function normalize_composition(array $composition, array $rule_index)
    {
        if (!$this->has_only_keys($composition, ['schemaVersion', 'label', 'nodes'])
            ...
            return new WP_Error('ocd_mcp_composition_invalid', 'La composición debe declarar schemaVersion=2 y una lista no vacía de nodos.');
        }
        ...
        foreach ($composition['nodes'] as $node) {
            if (!is_array($node)) {
                return new WP_Error('ocd_mcp_composition_node_invalid', 'Cada nodo de la composición debe ser un objeto.');
            }
            $normalized = $this->normalize_node($node, $rule_index, $seen_node_ids, $node_count, 0);
            if (is_wp_error($normalized)) {
                return $normalized;
            }
            $nodes[] = $normalized;
     ...
     * @return array<string, mixed>|WP_Error
     */
    private function normalize_node(array $node, array $rule_index, array &$seen_node_ids, int &$node_count, int $depth)
    {
        if ($depth > self::MAX_DEPTH
            || !$this->has_only_keys($node, ['id', 'kind', 'role', 'ruleIds', 'cadenceRuleId', 'children', 'content'])
            ...
            return new WP_Error('ocd_mcp_composition_node_invalid', 'Un nodo tiene una forma, tipo o profundidad no permitidos.');
        }
        if (isset($seen_node_ids[$node['id']])) {
            return new WP_Error('ocd_mcp_composition_node_duplicate', 'Los ids de nodos deben ser únicos.');
        }
        $seen_node_ids[$node['id']] = true;
        ++$node_count;
        if ($node_count > self::MAX_NODES) {
            return new WP_Error('ocd_mcp_composition_too_large', 'La composición excede el máximo de nodos permitido.');
        }

        $rule_ids = $node['ruleIds'] ?? [];
        if (!is_array($rule_ids) || !$this->is_list($rule_ids) || count($rule_ids) > 32) {
            return new WP_Error('ocd_mcp_composition_rule_reference_invalid', 'ruleIds debe ser una lista acotada.');
        }
        $normalized_rule_ids = [];
        foreach ($rule_ids as $rule_id) {
            if (!is_string($rule_id) || !isset($rule_index[$rule_id]) || $rule_index[$rule_id]['kind'] === 'cadence' || in_array($rule_id, $normalized_rule_ids, true)) {
                return new WP_Error('ocd_mcp_composition_rule_reference_invalid', 'ruleIds refiere una regla inexistente, cadencia directa o repetida.');
            }
        ...
        if ($cadence_rule_id !== '') {
            if (!is_string($cadence_rule_id) || !isset($rule_index[$cadence_rule_id]) || $rule_index[$cadence_rule_id]['kind'] !== 'cadence') {
                return new WP_Error('ocd_mcp_composition_cadence_reference_invalid', 'cadenceRuleId debe referir una regla cadence existente.');
            }
        }

        $children = $node['children'] ?? [];
        if (!is_array($children) || !$this->is_list($children) || count($children) > 80) {
            return new WP_Error('ocd_mcp_composition_children_invalid', 'children debe ser una lista acotada.');
        }
        $allows_children = in_array($node['kind'], ['section', 'header', 'footer', 'navigation', 'group', 'layout'], true);
        if (!$allows_children && $children !== []) {
            return new WP_Error('ocd_mcp_composition_children_invalid', 'Este tipo de nodo no admite children; usa una estructura o colección declarada.');
        }
        if ($allows_children && isset($node['content'])) {
            return new WP_Error('ocd_mcp_composition_content_invalid', 'Un nodo estructural usa children, no content.');
        }

        $normalized_children = [];
        foreach ($children as $child) {
            if (!is_array($child)) {
                return new WP_Error('ocd_mcp_composition_children_invalid', 'Cada child debe ser un nodo.');
            }
            $normalized_child = $this->normalize_node($child, $rule_index, $seen_node_ids, $node_count, $depth + 1);
        ...
        if (!is_array($content) || (!$allows_children && !isset($node['content']))) {
            return new WP_Error('ocd_mcp_composition_content_invalid', 'El contenido del nodo debe ser un objeto declarativo.');
        }
        if (!$allows_children) {
    ...
    private function normalize_node_content(string $kind, array $content)
    {
        switch ($kind) {
            case 'heading':
                if (!$this->has_only_keys($content, ['text', 'level']) || !isset($content['text'], $content['level'])
                    ...
                    return new WP_Error('ocd_mcp_heading_invalid', 'heading requiere text y level entre 1 y 6.');
                }
            ...
            case 'paragraph':
                if (!$this->has_only_keys($content, ['text']) || !isset($content['text']) || !$this->is_plain_text($content['text'], 5000)) {
                    return new WP_Error('ocd_mcp_paragraph_invalid', 'paragraph requiere texto plano acotado.');
                }
            ...
            case 'richText':
                if (!$this->has_only_keys($content, ['paragraphs']) || !isset($content['paragraphs']) || !is_array($content['paragraphs'])
                    ...
                    return new WP_Error('ocd_mcp_rich_text_invalid', 'richText requiere una lista acotada de párrafos de texto plano.');
                }
                foreach ($content['paragraphs'] as $paragraph) {
                    if (!$this->is_plain_text($paragraph, 5000)) {
                        return new WP_Error('ocd_mcp_rich_text_invalid', 'Cada párrafo de richText debe ser texto plano acotado.');
                    }
                }
            ...
            case 'audio':
                if (!$this->has_only_keys($content, ['sourceUrl', 'label']) || !isset($content['sourceUrl'])
                    ...
                    return new WP_Error('ocd_mcp_audio_invalid', 'audio requiere sourceUrl de activo Canvas y label opcional.');
                }
            ...
            case 'link':
                if (!$this->has_only_keys($content, ['label', 'href', 'target']) || !isset($content['label'], $content['href'])
                    ...
                    return new WP_Error('ocd_mcp_link_invalid', $kind . ' requiere label y href seguros.');
                }
            ...
            case 'list':
                if (!$this->has_only_keys($content, ['ordered', 'items']) || !isset($content['ordered'], $content['items'])
                    ...
                    return new WP_Error('ocd_mcp_list_invalid', 'list requiere ordered e items.');
                }
                foreach ($content['items'] as $item) {
                    if (!$this->is_plain_text($item, 1000)) {
                        return new WP_Error('ocd_mcp_list_invalid', 'Cada item de list debe ser texto plano.');
                    }
                }
            ...
            case 'form':
                if (!$this->has_only_keys($content, ['formSlug']) || !isset($content['formSlug']) || !is_string($content['formSlug'])) {
                    return new WP_Error('ocd_mcp_form_content_invalid', 'form requiere formSlug de un formulario publicado.');
                }
                $slug = sanitize_key($content['formSlug']);
                $available = array_column($this->available_forms(), 'slug');
                if ($slug === '' || !in_array($slug, $available, true)) {
                    return new WP_Error('ocd_mcp_form_not_published', 'formSlug no corresponde a un formulario Orugantt publicado.');
                }
            ...
            case 'shortcode':
                ...
                if (!$this->has_only_keys($content, ['tag', 'atts']) || !isset($content['tag'])
                    ...
                    return new WP_Error('ocd_mcp_shortcode_invalid', 'shortcode requiere tag con el nombre del shortcode (letras, números, guiones).');
                }
                $atts = [];
                if (isset($content['atts'])) {
                    if (!is_array($content['atts'])) {
                        return new WP_Error('ocd_mcp_shortcode_atts_invalid', 'atts debe ser un objeto de pares simples.');
                    }
                    foreach ($content['atts'] as $nombre => $valor) {
                        if (!is_string($nombre) || preg_match('/^[a-z0-9_-]{1,40}$/i', $nombre) !== 1) {
                            return new WP_Error('ocd_mcp_shortcode_atts_invalid', 'Los nombres de atts deben ser simples: letras, números, guiones.');
                        }
                        if (!is_string($valor) && !is_int($valor) && !is_float($valor) && !is_bool($valor)) {
                            return new WP_Error('ocd_mcp_shortcode_atts_invalid', 'Los valores de atts deben ser texto o número.');
                        }
            ...
            case 'separator':
                if ($content !== []) {
                    return new WP_Error('ocd_mcp_separator_invalid', 'separator no acepta content.');
                }
                return [];
            case 'chart':
        ...
        return new WP_Error('ocd_mcp_content_kind_invalid', 'El tipo de contenido no está disponible.');
    }
    ...
    private function normalize_image_content(array $content, bool $gallery_item)
    {
        ...
        if (!$this->has_only_keys($content, $allowed) || !isset($content['assetUrl'], $content['alt'])
            ...
            return new WP_Error('ocd_mcp_image_invalid', 'image requiere assetUrl resuelto y alt; caption es opcional.');
        }
        ...
        if (isset($content['rotation'])) {
            if (!is_int($content['rotation']) || !in_array($content['rotation'], [0, 90, 180, 270], true)) {
                return new WP_Error('ocd_mcp_image_invalid', 'rotation debe ser 0, 90, 180 o 270.');
            }
    ...
    private function normalize_dynamic_content(array $content)
    {
        if (!$this->has_only_keys($content, ['token', 'fallback']) || !isset($content['token'])
            ...
            return new WP_Error('ocd_mcp_dynamic_invalid', 'dynamic requiere un token Canvas disponible y fallback opcional.');
        }
    ...
    private function normalize_video_content(array $content)
    {
        ...
        if (!$this->has_only_keys($content, ['sourceUrl', 'posterUrl', 'caption', 'matte', 'pendingLabel'])
            ...
            return new WP_Error('ocd_mcp_video_invalid', 'video acepta sourceUrl de activo Canvas (o ninguno, y queda pendiente); poster, caption, matte y pendingLabel son opcionales.');
        }
        return [
    ...
    private function normalize_table_content(array $content)
    {
        if (!$this->has_only_keys($content, ['headers', 'rows']) || !isset($content['headers'], $content['rows'])
            ...
            return new WP_Error('ocd_mcp_table_content_invalid', 'table requiere headers y rows acotados.');
        }
        foreach ($content['headers'] as $header) {
            if (!$this->is_plain_text($header, 500)) {
                return new WP_Error('ocd_mcp_table_content_invalid', 'Cada encabezado de tabla debe ser texto plano.');
            }
        }
        $rows = [];
        foreach ($content['rows'] as $row) {
            if (!is_array($row) || !$this->is_list($row) || count($row) !== count($content['headers'])) {
                return new WP_Error('ocd_mcp_table_content_invalid', 'Cada fila debe coincidir con el número de headers.');
            }
            foreach ($row as $cell) {
                if (!$this->is_plain_text($cell, 1000)) {
                    return new WP_Error('ocd_mcp_table_content_invalid', 'Cada celda debe ser texto plano.');
                }
            }
    ...
    private function normalize_gallery_content(array $content)
    {
        if (!$this->has_only_keys($content, ['items']) || !isset($content['items']) || !is_array($content['items'])
            ...
            return new WP_Error('ocd_mcp_gallery_content_invalid', 'gallery requiere una lista acotada de items (primitivas image, video o dynamic).');
        }
        $items = [];
        foreach ($content['items'] as $item) {
            if (!is_array($item)) {
                return new WP_Error('ocd_mcp_gallery_content_invalid', 'Cada item de galería debe ser un objeto de primitiva.');
            }
            ...
            if (!is_string($kind) || !in_array($kind, ['image', 'video', 'dynamic'], true)) {
                return new WP_Error('ocd_mcp_gallery_content_invalid', 'kind de item de galería debe ser image, video o dynamic.');
            }
            $limpio = $item;
            unset($limpio['kind']);
            switch ($kind) {
    ...
    private function normalize_chart_content(array $content)
    {
        if (!$this->has_only_keys($content, ['type', 'labels', 'values', 'color']) || !isset($content['type'], $content['labels'], $content['values'])
            ...
            return new WP_Error('ocd_mcp_chart_invalid', 'chart requiere type, labels y values del mismo tamaño.');
        }
        foreach ($content['labels'] as $label) {
            if (!$this->is_plain_text($label, 100)) {
                return new WP_Error('ocd_mcp_chart_invalid', 'Las etiquetas de chart deben ser texto plano.');
            }
        }
        foreach ($content['values'] as $number) {
            if ((!is_int($number) && !is_float($number)) || !is_finite((float) $number)) {
                return new WP_Error('ocd_mcp_chart_invalid', 'Los valores de chart deben ser números finitos.');
            }
        }
        return [
            'type' => $content['type'],
    ...
    private function normalize_whatsapp_content(array $content)
    {
        ...
        if (!$this->has_only_keys($content, $allowed) || !isset($content['message'])
            ...
            return new WP_Error('ocd_mcp_whatsapp_invalid', 'whatsapp requiere message y ariaLabel opcional, texto plano acotado.');
        }
        $size = $content['size'] ?? 56;
        if (!is_int($size) || $size < 24 || $size > 200) {
            return new WP_Error('ocd_mcp_whatsapp_invalid', 'size debe ser un entero entre 24 y 200 (px).');
        }
        $icon_padding = $content['iconPadding'] ?? 14;
        if (!is_int($icon_padding) || $icon_padding < 0 || $icon_padding > 80) {
            return new WP_Error('ocd_mcp_whatsapp_invalid', 'iconPadding debe ser un entero entre 0 y 80 (px).');
        }
        ...
        if (!is_string($icon_color) || !$this->is_safe_css_color($icon_color)
            ...
            return new WP_Error('ocd_mcp_whatsapp_invalid', 'iconColor y backgroundColor deben ser colores CSS seguros (hex, rgb/hsl, var(--token), o nombre).');
        }
        $border_radius = $content['borderRadius'] ?? '999px';
        if (!is_string($border_radius) || !$this->is_safe_css_length_list($border_radius)) {
            return new WP_Error('ocd_mcp_whatsapp_invalid', 'borderRadius debe ser una longitud CSS segura (p. ej. 999px, 12px, 50%).');
        }
        return [
    ...
    private function normalize_anchor_rule(array $value)
    {
        ...
        if (!$this->has_only_keys($value, $allowed)) {
            return new WP_Error('ocd_mcp_anchor_rule_invalid', 'anchor admite solo edge, offset, animation y repeat.');
        }
        $edge = $value['edge'] ?? 'bottom';
        if (!is_string($edge) || !in_array($edge, self::ANCHOR_EDGES, true)) {
            return new WP_Error('ocd_mcp_anchor_rule_invalid', 'edge debe ser una de: ' . implode(', ', self::ANCHOR_EDGES) . '.');
        }
        $offset = $value['offset'] ?? 50;
        if ((!is_int($offset) && !is_float($offset)) || $offset < -100 || $offset > 100) {
            return new WP_Error('ocd_mcp_anchor_rule_invalid', 'offset debe ser un número entre -100 y 100 (% del tamaño del nodo).');
        }
        ...
        if (isset($value['animation'])) {
            if (!is_array($value['animation']) || !$this->has_only_keys($value['animation'], ['routine', 'level'])) {
                return new WP_Error('ocd_mcp_anchor_rule_invalid', 'animation admite solo routine y level.');
            }
            ...
            if (!is_string($routine) || !in_array($routine, self::ANCHOR_ROUTINES, true)
                ...
                return new WP_Error('ocd_mcp_anchor_rule_invalid', 'animation.routine debe ser rise|fade|scale|pop y level entre 1 y 3.');
            }
        ...
        // — no hardcodeado en el runtime, ver instalación en ocd-canvas-public.js.
        $repeat = true;
        if (isset($value['repeat'])) {
            if (!is_bool($value['repeat'])) {
                return new WP_Error('ocd_mcp_anchor_rule_invalid', 'repeat debe ser boolean.');
            }
            $repeat = $value['repeat'];
        }
    ...
    private function render_composition(array $composition, array $design)
    {
        ...
        $root_classes = ['ocd-mcp-page'];
        foreach ($design['rootRuleIds'] as $rule_id) {
            $root_classes[] = $this->rule_class($rule_id);
        }
        $label = $design['label'] !== '' ? $design['label'] : $design['designId'];
        $markup = '<main class="' . esc_attr(implode(' ', array_unique($root_classes))) . '" data-ocd-composition="v2" aria-label="'
    ...
    private function render_nodes(array $nodes, array $rule_index, array $node_ids, int $depth, string $cadence_rule_id, bool $wrap_columns)
    {
        ...
        foreach ($nodes as $index => $node) {
            ...
            $parts[] = $wrap_columns
                ? '<div class="ocd-column">' . $rendered . '</div>'
                : $rendered;
        }
    ...
    private function render_node(array $node, array $rule_ids, array $rule_index, array $node_ids, int $depth, int $item_index)
    {
        ...
        $classes = [
            'ocd-node',
            'ocd-node--' . sanitize_html_class($node['kind']),
            'ocd-node-id-' . sanitize_html_class($node['id']),
        ];
        $canvas_class = [
            'section' => 'ocd-section',
            'group' => 'ocd-group',
            'layout' => 'ocd-columns',
            'gallery' => 'ocd-dynamic-group',
            'form' => 'ocd-orugantt-form',
            'shortcode' => 'ocd-shortcode',
            'button' => 'ocd-button',
        ][$node['kind']] ?? '';
        if ($canvas_class !== '') {
        ...
        $attributes = array_merge([
            ...
            'data-ocd-node' => $node['id'],
        ], $behavior['attributes']);
        if ($node['role'] !== '') {
            $attributes['data-ocd-role'] = $node['role'];
        }
        ...
        switch ($node['kind']) {
            case 'section':
                ...
                return '<section ' . $attrs . '><div class="ocd-columns ocd-columns--single"><div class="ocd-column">' . $children . '</div></div></section>';
            case 'header':
            case 'footer':
                $children = $this->render_nodes($node['children'], $rule_index, $node_ids, $depth + 1, $child_cadence, false);
                if (is_wp_error($children)) {
                ...
                return '<' . $node['kind'] . ' ' . $attrs . '><div class="ocd-columns ocd-columns--single"><div class="ocd-column">' . $children . '</div></div></' . $node['kind'] . '>';
            case 'navigation':
                $children = $this->render_nodes($node['children'], $rule_index, $node_ids, $depth + 1, $child_cadence, false);
                if (is_wp_error($children)) {
                    return $children;
            ...
            case 'shortcode':
                ...
                $atts_json = $node['content']['atts'] === []
                    ? ''
                    : ' data-ocd-shortcode-atts="' . esc_attr((string) wp_json_encode($node['content']['atts'])) . '"';
                return '<div ' . $attrs . ' data-ocd-shortcode="' . esc_attr($node['content']['tag']) . '"'
                    . $atts_json . '></div>';
            case 'dynamic':
            ...
            case 'chart':
                ...
                return '<div ' . $attrs . ' data-ocd-behavior="chart" data-ocd-chart-type="' . esc_attr($node['content']['type'])
                    . '" data-ocd-chart-data="' . esc_attr((string) $data) . '" data-ocd-chart-color="'
                    . esc_attr($node['content']['color']) . '"></div>';
            case 'whatsapp':
                return $this->render_whatsapp($node['content'], $attrs, (string) $node['id']);
        }

        return new WP_Error('ocd_mcp_render_node_invalid', 'El tipo de nodo no puede materializarse en Canvas.');
    }
    ...
    private function node_behavior(array $node, array $rule_ids, array $rule_index, array $node_ids, int $item_index)
    {
        ...
        foreach ($rule_ids as $rule_id) {
            ...
            if ($rule['kind'] === 'gallery') {
                if ($gallery !== null) {
                    return new WP_Error('ocd_mcp_gallery_rule_conflict', 'Un nodo gallery sólo puede aplicar una regla de galería a la vez.');
                }
                $gallery = $rule['value'];
                continue;
            }
            if ($rule['kind'] === 'motion') {
                ...
                if ($motion['trigger'] === 'scroll') {
                    if ($runtime_behavior !== '') {
                        return new WP_Error('ocd_mcp_behavior_conflict', 'Un nodo no puede tener dos comportamientos runtime incompatibles.');
                    }
                    $runtime_behavior = 'reveal-on-scroll';
                    $attributes['data-ocd-behavior'] = 'reveal-on-scroll';
                    $attributes['data-ocd-reveal-class'] = 'is-revealed';
                    $attributes['data-ocd-reveal-threshold'] = (string) $motion['threshold'];
                    ...
                    $attributes['data-ocd-interaction'] = (string) $data;
                }
                if ($motion['stagger'] > 0) {
                    $attributes['style'] = '--ocd-motion-delay:' . ($motion['delay'] + $motion['stagger'] * $item_index) . 'ms';
                }
                continue;
            }
            if ($rule['kind'] === 'anchor') {
                ...
                $attributes['data-ocd-behavior'] = 'anchor';
                $attributes['data-ocd-anchor-reveal-transform'] = $anchor_style['revealedTransform'];
                $attributes['data-ocd-anchor-repeat'] = $rule['value']['repeat'] ? '1' : '0';
            ...
            if ($interaction['behavior'] === 'lightbox') {
                if ($node['kind'] !== 'gallery') {
                    return new WP_Error('ocd_mcp_lightbox_target_invalid', 'lightbox sólo puede aplicarse a un nodo gallery.');
                }
                $is_lightbox = true;
                continue;
            }
            ...
            $attributes['data-ocd-behavior'] = $interaction['behavior'];
            if ($interaction['behavior'] === 'scroll-threshold') {
                $attributes['data-ocd-scroll-threshold'] = (string) ($interaction['threshold'] ?? 40);
                $attributes['data-ocd-scrolled-class'] = $interaction['toggleClass'] ?? 'nav--scrolled';
            } elseif ($interaction['behavior'] === 'nav-toggle') {
                if (!in_array($interaction['targetId'], $node_ids, true)) {
                    return new WP_Error('ocd_mcp_interaction_target_missing', 'nav-toggle refiere un targetId que no está en esta composición.');
                }
                $attributes['data-ocd-toggle-target'] = '[data-ocd-node="' . $interaction['targetId'] . '"]';
                $attributes['data-ocd-toggle-class'] = $interaction['toggleClass'] ?? 'is-menu-open';
            } elseif ($interaction['behavior'] === 'carousel-basic') {
                if ($node['kind'] !== 'gallery') {
                    return new WP_Error('ocd_mcp_carousel_target_invalid', 'carousel-basic sólo puede aplicarse a un nodo gallery.');
                }
                $carousel = [
                ...
                $attributes['data-ocd-carousel-mode'] = $carousel['mode'];
                $attributes['data-ocd-carousel-slide-selector'] = '.ocd-carousel__slide';
                $attributes['data-ocd-carousel-active-class'] = 'is-active';
                $attributes['data-ocd-carousel-visible'] = (string) $carousel['visible'];
                $attributes['data-ocd-carousel-visible-mobile'] = (string) $carousel['visibleMobile'];
            }
        }

        if ($gallery !== null && ($gallery['controls'] ?? 'none') !== 'none' && $carousel === null) {
            return new WP_Error('ocd_mcp_gallery_controls_unavailable', 'Los controles de galería requieren una regla interaction.carousel-basic en el mismo nodo gallery.');
        }
    ...
    private function render_image(array $content, string $attrs): string
    {
        ...
        $rot_attrs = $rotation !== 0
            ? ' class="ocd-rot-' . $rotation . '" data-ocd-rotation="' . $rotation . '"'
            : '';
        $marco = $rotation === 90 || $rotation === 270 ? ' ocd-marco-girado' : '';
    ...
    private function render_video(array $content, string $attrs): string
    {
        ...
        if (($content['sourceUrl'] ?? '') === '') {
            ...
            return '<figure ' . $attrs . '><div class="ocd-video ocd-video--pendiente" role="img" aria-label="'
    ...
    /**
     * Video con transparencia real vía el compositor ocd-luma-matte (ya
     * registrado como primitiva del editor GrapesJS, ver
     * assets/js/ocd-editor-core.js `addType('ocd-luma-matte', ...)` y el
     * runtime público assets/js/ocd-luma-matte-video.js). El wrapper con
     * data-ocd-luma-matte="1" es lo único que ese runtime busca: video oculto
    ...
    {
        $caption = $content['caption'] !== '' ? '<figcaption>' . esc_html($content['caption']) . '</figcaption>' : '';
        $wrapper = '<div ' . $attrs . ' data-ocd-luma-matte="1">'
            . '<video class="ocd-luma-matte__video" muted playsinline autoplay loop preload="auto"><source src="' . esc_url($content['sourceUrl']) . '"></video>'
            . '<canvas class="ocd-luma-matte__canvas"></canvas>'
            . '</div>';
    ...
    private function render_gallery(array $node, string $attrs, array $behavior): string
    {
        ...
        foreach ($node['content']['items'] as $index => $item) {
            ...
            $clases = 'class="ocd-mcp-gallery__item ocd-carousel__slide' . $active . '"';
        ...
        if ((is_array($gallery) && ($gallery['mode'] ?? '') === 'carousel')
            ...
            $body = '<div class="ocd-carousel__track">' . $body . '</div>';
        }
        $controls = is_array($gallery) ? ($gallery['controls'] ?? 'none') : 'arrows';
        if ($behavior['carousel'] !== null && $controls !== 'none') {
            $body .= '<div class="ocd-mcp-gallery__controls"><button type="button" data-ocd-carousel-prev aria-label="Anterior">‹</button>'
                . '<button type="button" data-ocd-carousel-next aria-label="Siguiente">›</button></div>';
            if ($controls === 'arrows-and-dots') {
                $body .= '<div class="ocd-mcp-gallery__dots" data-ocd-carousel-dots></div>';
            }
        }
        if ($behavior['lightbox']) {
            $source = '.ocd-node-id-' . sanitize_html_class($node['id']);
            $body .= '<div class="ocd-mcp-lightbox" data-ocd-behavior="lightbox" data-ocd-lightbox-source="'
                . esc_attr($source) . '" data-ocd-lightbox-image-selector="img" data-ocd-lightbox-open-class="is-open" aria-hidden="true">'
                . '<button type="button" data-ocd-lightbox-close aria-label="Cerrar">×</button>'
                . '<button type="button" data-ocd-lightbox-prev aria-label="Anterior">‹</button>'
                . '<img data-ocd-lightbox-img alt=""><button type="button" data-ocd-lightbox-next aria-label="Siguiente">›</button></div>';
        }
    ...
    private function render_dynamic(array $content, string $attrs): string
    {
        ...
        if ($token === 'featured_image' || strncmp($token, 'acf_image:', 10) === 0) {
            return '<img ' . $attrs . ' data-ocd-dynamic="' . esc_attr($token) . '" src="" alt="">';
        }
        if ($token === 'permalink') {
            return '<a ' . $attrs . ' data-ocd-dynamic="permalink" href="#">' . esc_html($fallback) . '</a>';
        }
        return '<span ' . $attrs . ' data-ocd-dynamic="' . esc_attr($token) . '">' . esc_html($fallback) . '</span>';
    }

    /** @param array<string, mixed> $content */
    private function render_whatsapp(array $content, string $attrs, string $node_id): string
    {
        $number = preg_replace('/[^0-9]/', '', (string) get_option('ocd_whatsapp_number', ''));
        ...
        return '<a ' . $attrs . ' data-ocd-whatsapp-message="' . esc_attr($content['message'])
    ...
    {
        return "\n"
            . ".ocd-mcp-page{color:#1d2327;background:#fff;line-height:1.5;}\n"
            . ".ocd-mcp-page *{box-sizing:border-box;}\n"
            . ".ocd-mcp-page img,.ocd-mcp-page video,.ocd-mcp-page audio{display:block;max-width:100%;}\n"
            . ".ocd-section{width:100%;padding:48px 24px;position:relative;}\n"
            . ".ocd-columns{display:grid;grid-template-columns:minmax(0,1fr);gap:24px;width:100%;}\n"
            . ".ocd-column{min-width:0;}\n"
            . ".ocd-group{display:grid;gap:16px;}\n"
            . ".ocd-node--layout{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:24px;}\n"
            . ".ocd-node--gallery{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px;}\n"
            . ".ocd-mcp-gallery__item{margin:0;min-width:0;overflow:hidden;}\n"
            . ".ocd-mcp-gallery__item img{width:100%;height:100%;object-fit:cover;}\n"
            . ".ocd-mcp-gallery__item video,.ocd-mcp-gallery__item figure{width:100%;height:100%;margin:0;}\n"
            . ".ocd-mcp-gallery__item video{object-fit:cover;display:block;}\n"
            . ".ocd-video--pendiente{display:flex;align-items:center;justify-content:center;width:100%;aspect-ratio:16/9;background:rgba(0,0,0,0.06);opacity:.75;font-size:.8em;letter-spacing:.06em;text-transform:uppercase;}\n"
            ...
            . ".ocd-rot-180{transform:rotate(180deg);}\n"
            . ".ocd-marco-girado{position:relative;overflow:hidden;container-type:size;}\n"
            . ".ocd-marco-girado>.ocd-rot-90,.ocd-marco-girado>.ocd-rot-270{position:absolute;top:50%;left:50%;width:100cqh;height:100cqw;max-width:none;object-fit:cover;}\n"
            ...
            . "@supports not (width:100cqh){.ocd-marco-girado>.ocd-rot-90,.ocd-marco-girado>.ocd-rot-270{position:static;width:100%;height:auto;transform:rotate(90deg);}}\n"
            . ".ocd-mcp-gallery__controls{grid-column:1/-1;display:flex;gap:8px;}\n"
            . ".ocd-mcp-lightbox{display:none;position:fixed;inset:0;z-index:9999;padding:32px;background:rgba(0,0,0,.86);align-items:center;justify-content:center;gap:12px;}\n"
            . ".ocd-mcp-lightbox.is-open{display:flex;}\n"
            . ".ocd-mcp-lightbox img{max-width:min(80vw,1100px);max-height:80vh;}\n"
            . ".ocd-node--table{overflow-x:auto;}\n"
            . ".ocd-node--table table{border-collapse:collapse;width:100%;}\n"
            . ".ocd-node--table th,.ocd-node--table td{padding:.75rem;text-align:start;}\n"
            . ".ocd-button{display:inline-flex;align-items:center;justify-content:center;padding:.75rem 1.25rem;text-decoration:none;}\n"
            . "@media(max-width:767px){.ocd-section{padding:32px 16px;}.ocd-node--layout,.ocd-node--gallery{grid-template-columns:minmax(0,1fr);}}\n";
    }
    ...
    private function css_for_rule(array $rule): string
    {
        ...
        switch ($rule['kind']) {
            case 'color':
                $css = '--ocd-color-' . sanitize_html_class($value['role']) . ':' . $value['color'] . ';';
    ...
    private function media_css(string $selector, array $value): string
    {
        ...
        // con matte (ocd-luma-matte): su pieza visible es el <canvas>, el
        ...
        if (isset($value['overlayColor'])) {
            $overlay_target = $selector . '::after,' . $selector . ' .ocd-mcp-gallery__item::after';
            $css .= $selector . ',' . $selector . ' .ocd-mcp-gallery__item{position:relative;isolation:isolate;}'
    ...
    private function button_css(array $value): string
    {
        ...
        $colors = [
            'primary' => ['background' => 'var(--ocd-color-accent,#2271b1)', 'foreground' => '#fff'],
            'secondary' => ['background' => 'var(--ocd-color-surface,#f0f0f1)', 'foreground' => 'var(--ocd-color-ink,#1d2327)'],
    ...
    private function gallery_css(string $selector, array $value): string
    {
        ...
        if ($value['mode'] === 'masonry') {
            $css = $selector . '{display:block;columns:' . $columns . ';column-gap:' . $gap . ';}'
                . $selector . ' .ocd-mcp-gallery__item{break-inside:avoid;margin-bottom:' . $gap . ';}';
        } elseif ($value['mode'] === 'carousel') {
            $css = $selector . '{display:block;}'
                . $selector . ' .ocd-carousel__track{display:flex;overflow-x:auto;gap:' . $gap . ';scroll-snap-type:x mandatory;}'
                . $selector . ' .ocd-carousel__track .ocd-mcp-gallery__item{flex:0 0 calc((100% - ' . (($columns - 1) * 1) . '*' . $gap . ')/' . $columns . ');scroll-snap-align:start;}';
        } else {
            $css = $selector . '{display:grid;grid-template-columns:repeat(' . $columns . ',minmax(0,1fr));gap:' . $gap . ';}';
            if ($value['mode'] === 'metro') {
                $css .= $selector . ' .ocd-mcp-gallery__item:nth-child(5n+1){grid-column:span 2;grid-row:span 2;}';
            }
        }
        if (isset($value['mobileColumns'])) {
            if ($value['mode'] === 'carousel') {
                $mobile_columns = $value['mobileColumns'];
                $css .= '@media(max-width:767px){' . $selector . ' .ocd-carousel__track .ocd-mcp-gallery__item{flex-basis:calc((100% - '
    ...
    private function table_css(string $selector, array $value): string
    {
        ...
        if (($value['header'] ?? '') === 'accent') {
            $css .= $selector . ' th{background:var(--ocd-color-accent,#2271b1);color:#fff;}';
    ...
    private function normalize_form_theme($value)
    {
        if (!is_array($value) || $value === []) {
            return new WP_Error('ocd_mcp_form_rule_invalid', 'theme debe declarar al menos una variable de diseño.');
        }
        if (count($value) > 40) {
            return new WP_Error('ocd_mcp_form_rule_invalid', 'theme declara demasiadas variables.');
        }
        ...
        foreach ($value as $clave => $bruto) {
            if (!is_string($clave) || !isset($contrato[$clave])) {
                return new WP_Error(
                    'ocd_mcp_form_rule_invalid',
                    'theme: "' . (is_string($clave) ? $clave : '?') . '" no es una variable de diseño del formulario.'
                );
            }
            if (!is_string($bruto) || trim($bruto) === '') {
                return new WP_Error('ocd_mcp_form_rule_invalid', 'theme: el valor de "' . $clave . '" debe ser texto.');
            }
            ...
            if (!$valido) {
                return new WP_Error('ocd_mcp_form_rule_invalid', 'theme: el valor de "' . $clave . '" no es válido para un valor de tipo ' . $tipo . '.');
            }
    ...
    private function motion_css(string $selector, array $value): string
    {
        ...
        $transition = 'opacity ' . $value['duration'] . 'ms ' . $value['easing'] . ' var(--ocd-motion-delay,' . $delay . '),transform '
            . $value['duration'] . 'ms ' . $value['easing'] . ' var(--ocd-motion-delay,' . $delay . ');';
    ...
    {
        return 'ocd-rule--' . sanitize_html_class($rule_id);
    }
