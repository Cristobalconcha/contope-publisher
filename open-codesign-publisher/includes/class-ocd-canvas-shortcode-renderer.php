 ...
 *
 *     <div data-ocd-shortcode="instagram-feed"
 *          data-ocd-shortcode-atts='{"feed":"1"}'></div>
 *
 ...
 * editando esta clase.
 */
final class OCD_Canvas_Shortcode_Renderer
{
    /**
    ...
    {
        $lista = apply_filters('ocd_canvas_shortcodes_permitidos', self::PERMITIDOS);
        if (!is_array($lista)) {
    ...
    {
        if (strpos($html, 'data-ocd-shortcode') === false) {
            return $html;
        }

        $resultado = preg_replace_callback(
            '/<div\b([^>]*\bdata-ocd-shortcode=[^>]*)>\s*<\/div>/i',
    ...
    private function render_marcador(string $atributos): string
    {
        ...
        if (preg_match('/data-ocd-shortcode=("([^"]*)"|\'([^\']*)\')/i', $atributos, $m) !== 1) {
            return '';
        }
    ...
    /**
     * Convierte data-ocd-shortcode-atts (un objeto JSON) en los atributos del
    ...
    {
        if (preg_match('/data-ocd-shortcode-atts=("([^"]*)"|\'([^\']*)\')/i', $atributos, $m) !== 1) {
            return '';
        }
