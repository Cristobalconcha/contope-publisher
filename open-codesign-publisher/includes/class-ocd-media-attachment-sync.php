 ...
 * `wp-content/uploads/open-codesign/`).
 *
 * La resolución de URLs para renderizar NO cambia: sigue a cargo de
 * `OCD_Canvas_Asset_Resolver`. Esta clase solo agrega el adjunto para que la
 ...
 * archivos.
 */
final class OCD_Media_Attachment_Sync
{
    /**
    ...
    public function sync_file_with_status(string $absolute_path)
    {
        ...
        if (!is_file($absolute_path)) {
            return new WP_Error('ocd_media_sync_missing_file', "No existe el archivo $absolute_path.");
        }

        $upload_dir = wp_get_upload_dir();
        if (!empty($upload_dir['error'])) {
            return new WP_Error('ocd_media_sync_upload_dir', (string) $upload_dir['error']);
        }

        $basedir = wp_normalize_path((string) $upload_dir['basedir']);
        $relative = $this->relative_to_uploads($absolute_path, $basedir);
        if ($relative === null) {
            return new WP_Error('ocd_media_sync_outside_uploads', "El archivo $absolute_path no está dentro del directorio de uploads.");
        }
    ...
    private function create_attachment(string $absolute_path, string $relative)
    {
        ...
        if ($attachment_id === 0) {
            return new WP_Error('ocd_media_sync_insert_failed', "No fue posible insertar el adjunto para $relative.");
        }
