...
final class OCD_Package_Validator
{
    ...
    public function validate(array $document)
    {
        if (($document['schemaVersion'] ?? null) !== 0) {
            return new WP_Error('ocd_schema_version', 'La versión del formato no es compatible.');
        }

        $project = $document['project'] ?? null;
        if (!is_array($project) || !$this->valid_id($project['id'] ?? null) || !$this->text($project['name'] ?? null)) {
            return new WP_Error('ocd_project', 'Los metadatos del proyecto son inválidos.');
        }

        $pages = $document['pages'] ?? null;
        if (!is_array($pages) || count($pages) < 1 || count($pages) > self::MAX_PAGES) {
            return new WP_Error('ocd_pages', 'El documento debe contener entre 1 y 50 páginas.');
        }
        ...
        foreach ($pages as $page) {
            if (!is_array($page)) {
                return new WP_Error('ocd_page', 'La definición de una página es inválida.');
            }
            $page_id = $page['id'] ?? null;
            $slug = $page['slug'] ?? null;
            if (!$this->valid_id($page_id) || isset($page_ids[$page_id])) {
                return new WP_Error('ocd_page_id', 'Los IDs de página deben ser válidos y únicos.');
            }
            if (!$this->text($page['title'] ?? null) || !is_string($slug) || sanitize_title($slug) !== $slug || isset($slugs[$slug])) {
                return new WP_Error('ocd_page_fields', 'Cada página necesita título y slug válido y único.');
            }
            if (!isset($page['nodes']) || !is_array($page['nodes'])) {
                return new WP_Error('ocd_page_nodes', 'Cada página debe contener una lista de nodos.');
            }
    ...
    private function validate_nodes(array $nodes, array &$node_ids, int &$node_count, ?string $parent_type)
    {
        foreach ($nodes as $node) {
            ...
            if ($node_count > self::MAX_NODES) {
                return new WP_Error('ocd_node_limit', 'El documento supera el límite de nodos.');
            }
            if (!is_array($node) || !$this->valid_id($node['id'] ?? null)) {
                return new WP_Error('ocd_node_id', 'Cada nodo necesita un ID válido.');
            }
            if (isset($node_ids[$node['id']])) {
                return new WP_Error('ocd_duplicate_node', 'Los IDs de nodo deben ser únicos en el proyecto.');
            }
            $node_ids[$node['id']] = true;

            $type = $node['type'] ?? null;
            if (!is_string($type) || !in_array($type, self::ALLOWED_TYPES, true)) {
                return new WP_Error('ocd_node_type', 'El documento contiene un tipo de nodo desconocido.');
            }
            if ($type === 'column' && $parent_type !== 'columns') {
                return new WP_Error('ocd_column_parent', 'Una columna debe pertenecer directamente a un nodo columns.');
            }
            ...
            if (!is_array($children)) {
                return new WP_Error('ocd_children', 'La colección children debe ser una lista.');
            }
