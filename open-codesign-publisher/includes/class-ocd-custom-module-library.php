 ...
 * alcance (ver class-ocd-theme-definitions.php para el mismo patrón de
 * opción-JSON usado en este plugin).
 *
 * Cada entrada tiene un `id` estable en forma de slug (`ocd-custom-<uuid4>`),
 ...
 * (ver la nota de `document_id` en OCD_Canvas_Editor_Admin), y una biblioteca
 * de módulos reusables viaja entre sitios en un paquete exactamente igual que
 * los documentos Canvas.
 */
final class OCD_Custom_Module_Library
{
    public const OPTION_KEY = 'ocd_custom_modules';

    /** Límites defensivos, en el mismo espíritu que OCD_Canvas_Document_Sanitizer. */
     ...
     * OCD_Canvas_Document_Sanitizer::sanitize_html()/sanitize_css(), igual que
    ...
    public function save(string $label, string $category, string $html, string $css): array
    {
        ...
        $entry = [
            'id' => 'ocd-custom-' . wp_generate_uuid4(),
            'label' => $label,
            'category' => $category,
            'html' => $html,
            'css' => $css,
