 ...
 * `ocd_patch_html`/`ocd_patch_css` son la excepción histórica (texto exacto,
 * sin tocar projectData) y `ocd_grapes_edit_node` es la vía real: dirige un
 * selector CSS al editor Grapes de verdad vía `tools/ocd-headless-node-edit.mjs`
 ...
 */
final class OCD_Canvas_MCP_Service
{
    ...
    public function __construct(
        ...
        private OCD_Canvas_MCP_Recipe_Compiler $compiler,
    ...
    public function capabilities(): array
    {
        ...
        return [
            'adapter' => ['name' => 'open-codesign-canvas', 'version' => self::ADAPTER_VERSION],
            'operations' => [
            ...
            'workflow' => [
                ...
                '5' => 'Ejecuta ocd_preview_canvas_composition y revisa su evidencia.',
                '6' => 'Ejecuta ocd_apply_canvas_composition con el previewId exacto; Canvas crea snapshot.',
                '7' => 'Revisa visualmente en Canvas y publica sólo con ocd_publish_canvas_page si corresponde.',
            ],
        ];
    }
    ...
    public function canvas_page_state(int $page_id)
    {
        ...
        if ($document === null) {
            return new WP_Error('ocd_mcp_canvas_document_missing', 'La página Canvas existe, pero su documento persistente no está disponible.');
        }
        $regions = [];
     ...
     * cambiar) a ocd_preview_canvas_composition — así se puede editar un nodo
    ...
    public function read_canvas_composition(int $page_id)
    {
        ...
        if (!$result['found']) {
            return [
                ...
                'notice' => 'Esta página no tiene una composición MCP registrada (se construyó, o se editó después, fuera de ocd_apply_canvas_composition). No hay árbol para leer; una composición nueva reemplaza todo el contenido actual.',
            ];
        }
        return [
            'page' => $page,
            ...
            'next' => 'Editá sólo el/los nodo(s) que quieras por su id (o las reglas que referencian) y resubmití composition+design completos, con expectedRevision = revision, a ocd_preview_canvas_composition.',
        ];
    }
    ...
    public function create_canvas_page(string $title)
    {
        ...
        if ($page === null || $document === null) {
            return new WP_Error('ocd_mcp_canvas_create_incomplete', 'La página Canvas se creó, pero no pudo verificarse su vínculo persistente.');
        }
        return [
            'page' => $page,
            'document' => $document,
            'next' => 'Resuelve activos y formularios necesarios; luego solicita ocd_preview_canvas_composition antes de aplicar.',
        ];
    }
    ...
    public function preview_canvas_composition(int $page_id, string $document_id, int $expected_revision, array $composition, array $design)
    {
        ...
        return [
            ...
            'visualPreviewPersisted' => false,
            'next' => 'Si la evidencia es correcta, usa ocd_apply_canvas_composition con este previewId; la operación crea un snapshot antes de escribir.',
        ];
    }
    ...
    public function apply_canvas_composition(int $page_id, string $document_id, int $expected_revision, string $preview_id, array $composition, array $design)
    {
        ...
        if (!hash_equals($expected_preview_id, $preview_id)) {
            return new WP_Error('ocd_mcp_preview_mismatch', 'previewId no corresponde exactamente a esta composición, destino y revisión. Vuelve a previsualizar antes de aplicar.');
        }
        ...
        $page = $this->publisher->describe_canvas_page($page_id);
        if ($page === null) {
            return new WP_Error('ocd_mcp_canvas_page_unavailable', 'La página Canvas no pudo leerse después de aplicar la composición.');
        }
        return [
            'page' => $page,
    ...
    public function tool_definitions(): array
    {
        ...
        return [
            ...
            [
                'name' => 'ocd_get_canvas_page_state',
            ...
            [
                'name' => 'ocd_read_canvas_composition',
            ...
            [
                'name' => 'ocd_resolve_canvas_assets',
                'title' => 'Resolver activos Canvas',
                'description' => 'Busca referencias entre activos ya gestionados por Open CoDesign. No sube archivos ni expone rutas locales.',
                'inputSchema' => [
                    'type' => 'object',
                    'properties' => [
                        'references' => ['type' => 'array', 'minItems' => 1, 'maxItems' => OCD_Canvas_Asset_Resolver::MAX_REFERENCES, 'items' => ['type' => 'string', 'minLength' => 1, 'maxLength' => 2048]],
                    ],
                    'required' => ['references'],
                    'additionalProperties' => false,
                ],
            ...
            [
                'name' => 'ocd_preview_canvas_composition',
            ...
            [
                'name' => 'ocd_apply_canvas_composition',
            ...
            [
                ...
                'description' => 'Busca un texto exacto en _ocd_canvas_css y lo reemplaza. Rechaza si el texto no aparece o aparece más de una vez (ambiguo) — no escribe nada en ese caso. No toca HTML ni projectData.',
                'inputSchema' => [
                    'type' => 'object',
                    'properties' => array_merge($target, [
            ...
            [
                ...
                'description' => 'Busca un texto exacto en _ocd_canvas_html y lo reemplaza (o lo elimina, si replace es ""). Rechaza si el texto no aparece o aparece más de una vez (ambiguo) — no escribe nada en ese caso. No toca CSS ni projectData.',
                'inputSchema' => [
                    'type' => 'object',
                    'properties' => array_merge($target, [
                        'search' => ['type' => 'string', 'minLength' => 1],
            ...
            [
                ...
                'inputSchema' => [
                    'type' => 'object',
                    'properties' => array_merge($target, [
                        'selector' => ['type' => 'string', 'minLength' => 1, 'description' => 'Selector CSS del nodo a editar, ej. ".hero-family" o "[data-ocd-node=\'wa-section-2\']". Debe resolver a exactamente un nodo.'],
                        'mutation' => [
                            'type' => 'object',
                            'properties' => [
            ...
            [
                'name' => 'ocd_read_canvas_document',
                'title' => 'Devuelve projectData/html/css crudos de un documento',
                'description' => 'Lectura cruda del documento Canvas (projectData, html, css y revisión). Existe para el puente headless a Grapes cuando este corre FUERA del servidor (el hosting no tiene Node): el cliente lee el documento con esta herramienta, lo edita con el motor real de GrapesJS en una máquina que sí puede ejecutarlo, y devuelve el resultado con ocd_write_canvas_document.',
                'inputSchema' => [
                    'type' => 'object',
                    'properties' => [
            ...
            [
                'name' => 'ocd_write_canvas_document',
                'title' => 'Guarda projectData/html/css ya producidos por el motor real de Grapes',
                'description' => 'Contraparte de escritura de ocd_read_canvas_document. Acepta projectData/html/css tal cual los devolvió GrapesJS (editor.getProjectData()/getHtml()/getCss()) tras una edición hecha con su API real, los sanea y los guarda de forma consistente entre sí. No es para HTML escrito a mano: para eso están ocd_patch_html/ocd_patch_css, que además dejan projectData desincronizado.',
                'inputSchema' => [
                    'type' => 'object',
                    'properties' => array_merge($target, [
            ...
            [
                'name' => 'ocd_sync_project_data',
                'title' => 'Reconstruye projectData a partir del HTML/CSS vigentes',
                'description' => 'ocd_patch_html y ocd_patch_css nunca tocan projectData (el árbol que lee el editor visual GrapesJS) — tras usarlos, projectData queda desincronizado del HTML/CSS real. Si en ese estado se abre el editor visual y se guarda ahí, pisaría los cambios hechos por MCP. Este tool cierra esa brecha: empaqueta el HTML/CSS actuales como projectData (mismo formato que usa una composición MCP normal) sin alterar el HTML ni el CSS. Úsalo después de una tanda de ocd_patch_html/ocd_patch_css, antes de que alguien abra el editor visual.',
                'inputSchema' => [
                    'type' => 'object',
                    'properties' => $target,
            ...
            [
                'name' => 'ocd_activate_section_whatsapp',
            ...
            [
                'name' => 'ocd_patch_geo_places',
                'title' => 'Actualizar lugares de un geo-map ya publicado',
                'description' => 'Reemplaza el atributo data-ocd-geo-places de un geo-map ya existente en una página Canvas, sin tocar el resto del HTML/CSS/projectData. No crea un geo-map nuevo; el documento debe tener uno ya construido.',
                'inputSchema' => [
                    'type' => 'object',
                    'properties' => array_merge($target, [
                        'places' => [
    ...
    {
        if ($tool_name === 'ocd_get_capabilities') {
            return $this->reject_unexpected_arguments($arguments) ?? $this->capabilities();
        }
        if ($tool_name === 'ocd_list_canvas_pages') {
            $error = $this->reject_unexpected_arguments($arguments);
            return $error ?? ['pages' => $this->list_canvas_pages()];
        }
        if ($tool_name === 'ocd_get_canvas_page_state') {
            if (!$this->has_only_keys($arguments, ['pageId']) || !isset($arguments['pageId']) || !is_int($arguments['pageId']) || $arguments['pageId'] < 1) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'pageId debe ser un entero positivo y el único argumento.');
            }
            return $this->canvas_page_state($arguments['pageId']);
        }
        if ($tool_name === 'ocd_read_canvas_composition') {
            if (!$this->has_only_keys($arguments, ['pageId']) || !isset($arguments['pageId']) || !is_int($arguments['pageId']) || $arguments['pageId'] < 1) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'pageId debe ser un entero positivo y el único argumento.');
            }
            return $this->read_canvas_composition($arguments['pageId']);
        }
        if ($tool_name === 'ocd_resolve_canvas_assets') {
            if (!$this->has_only_keys($arguments, ['references']) || !isset($arguments['references']) || !is_array($arguments['references']) || !$this->is_list($arguments['references']) || count($arguments['references']) < 1 || count($arguments['references']) > OCD_Canvas_Asset_Resolver::MAX_REFERENCES) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'references debe ser una lista de 1 a ' . OCD_Canvas_Asset_Resolver::MAX_REFERENCES . ' referencias.');
            }
            foreach ($arguments['references'] as $reference) {
                if (!is_string($reference) || $reference === '' || strlen($reference) > 2048) {
                    return new WP_Error('ocd_mcp_invalid_arguments', 'Cada referencia de activo debe ser texto no vacío de hasta 2048 caracteres.');
                }
            }
            return $this->resolve_canvas_assets($arguments['references']);
        }
        if ($tool_name === 'ocd_list_canvas_forms') {
            return $this->reject_unexpected_arguments($arguments) ?? $this->list_canvas_forms();
        }
        if ($tool_name === 'ocd_create_canvas_page') {
            if (!$this->has_only_keys($arguments, ['title']) || !array_key_exists('title', $arguments)) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'title es obligatorio para crear una página Canvas.');
            }
            $title = $this->normalize_page_title($arguments['title']);
            return is_wp_error($title) ? $title : $this->create_canvas_page($title);
        }
        if ($tool_name === 'ocd_preview_canvas_composition') {
        ...
        if ($tool_name === 'ocd_apply_canvas_composition') {
        ...
        if ($tool_name === 'ocd_patch_css') {
            ...
            if (!isset($arguments['search'], $arguments['replace']) || !is_string($arguments['search']) || !is_string($arguments['replace']) || $arguments['search'] === '') {
                return new WP_Error('ocd_mcp_invalid_arguments', 'search y replace deben ser texto; search no puede ser vacío.');
            }
            return $this->patch_css($target['pageId'], $target['documentId'], $target['expectedRevision'], $arguments['search'], $arguments['replace']);
        }
        if ($tool_name === 'ocd_patch_html') {
            $target = $this->target_arguments($arguments, ['search', 'replace']);
            if (is_wp_error($target)) {
                return $target;
            }
        ...
        if ($tool_name === 'ocd_grapes_edit_node') {
            ...
            if (!isset($arguments['selector']) || !is_string($arguments['selector']) || $arguments['selector'] === '') {
                return new WP_Error('ocd_mcp_invalid_arguments', 'selector debe ser texto no vacío.');
            }
        ...
        if ($tool_name === 'ocd_read_canvas_document') {
            ...
            if ($page_id < 0 || $document_id === '') {
                return new WP_Error('ocd_mcp_invalid_arguments', 'pageId y documentId son obligatorios.');
            }
            return $this->read_canvas_document($page_id, $document_id);
        }
        if ($tool_name === 'ocd_write_canvas_document') {
            ...
            foreach (['projectData', 'html', 'css'] as $key) {
                if (!isset($arguments[$key]) || !is_string($arguments[$key])) {
                    return new WP_Error('ocd_mcp_invalid_arguments', $key . ' debe ser texto.');
                }
            }
        ...
        if ($tool_name === 'ocd_sync_project_data') {
        ...
        if ($tool_name === 'ocd_activate_section_whatsapp') {
            ...
            if (!is_string($position)) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'position debe ser una cadena.');
            }
            $style = $arguments['style'] ?? [];
            if (!is_array($style)) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'style debe ser un objeto.');
            }
            ...
            if ($anchor_selector !== null && (!is_string($anchor_selector) || preg_match('/^\.[a-zA-Z_][a-zA-Z0-9_-]*$/', $anchor_selector) !== 1)) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'anchorSelector debe ser una clase simple, p. ej. ".wrap".');
            }
        ...
        if ($tool_name === 'ocd_patch_geo_places') {
            ...
            if (!isset($arguments['places']) || !is_array($arguments['places']) || !$this->is_list($arguments['places']) || count($arguments['places']) < 1) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'places debe ser una lista no vacía de lugares.');
            }
            return $this->patch_geo_places($target['pageId'], $target['documentId'], $target['expectedRevision'], $arguments['places']);
        }
        if ($tool_name === 'ocd_set_whatsapp_number') {
            if (!$this->has_only_keys($arguments, ['number']) || !isset($arguments['number']) || !is_string($arguments['number']) || preg_match('/^[0-9]{8,15}$/', $arguments['number']) !== 1) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'number debe ser una cadena de 8 a 15 dígitos, con código de país y sin +.');
            }
            update_option('ocd_whatsapp_number', $arguments['number'], false);
            return ['number' => $arguments['number'], 'saved' => true];
        }
        if ($tool_name === 'ocd_publish_canvas_page') {
            $target = $this->target_arguments($arguments, []);
            if (is_wp_error($target)) {
                return $target;
            }
        ...
        return new WP_Error('ocd_mcp_tool_not_found', 'La herramienta MCP solicitada no existe.');
    }

    /**
     * Reemplaza SOLO el atributo data-ocd-geo-places de _ocd_canvas_html,
    ...
    private function patch_geo_places(int $page_id, string $document_id, int $expected_revision, array $places)
    {
        ...
        if (!preg_match('/data-ocd-geo-places="([^"]*)"/', $html, $matches)) {
            return new WP_Error('ocd_mcp_geo_places_not_found', 'Esta página no tiene un geo-map con data-ocd-geo-places para actualizar.');
        }
        $decoded_current = json_decode(html_entity_decode($matches[1], ENT_QUOTES), true);
        if (!is_array($decoded_current)) {
            return new WP_Error('ocd_mcp_geo_places_invalid', 'El data-ocd-geo-places actual no es JSON válido; no se toca por seguridad.');
        }
        $new_json = wp_json_encode(array_values($places), JSON_UNESCAPED_UNICODE);
        if (!is_string($new_json)) {
            return new WP_Error('ocd_mcp_geo_places_encode', 'No se pudo codificar la nueva lista de lugares.');
        }
        $new_html = str_replace(
            'data-ocd-geo-places="' . $matches[1] . '"',
            'data-ocd-geo-places="' . esc_attr($new_json) . '"',
            $html
        );
     ...
     * ocd_apply_canvas_composition construiría) como hijo directo de cada
    ...
    private function activate_section_whatsapp(int $page_id, string $document_id, int $expected_revision, string $position, array $style = [], ?string $anchor_selector = null)
    {
        ...
        if (strpos($html, 'data-wa-msg') === false) {
            return new WP_Error('ocd_mcp_no_whatsapp_sections', 'Esta página no tiene secciones con data-wa-msg.');
        }

        libxml_use_internal_errors(true);
        $dom = new DOMDocument('1.0', 'UTF-8');
        $dom->loadHTML('<?xml encoding="utf-8"?><div id="ocd-mcp-root">' . $html . '</div>', LIBXML_NOERROR | LIBXML_NOWARNING);
        libxml_clear_errors();
        $xpath = new DOMXPath($dom);
        $sections = $xpath->query('//*[@data-wa-msg]');
        if ($sections === false || $sections->length === 0) {
            return new WP_Error('ocd_mcp_no_whatsapp_sections', 'No se encontraron elementos con data-wa-msg.');
        }
        ...
        foreach ($sections as $index => $section) {
            ...
            if ($anchor === null) {
                return new WP_Error('ocd_mcp_whatsapp_anchor_not_found', 'No se encontró el elemento ancla (fila) dentro de una de las secciones con data-wa-msg.');
            }
            ...
            $already = $xpath->query('.//*[contains(concat(" ", normalize-space(@class), " "), " ocd-node--whatsapp ")]', $section);
            if ($already !== false) {
            ...
            $fragmentDoc->loadHTML('<?xml encoding="utf-8"?><div id="ocd-mcp-fragment">' . $module_html . '</div>', LIBXML_NOERROR | LIBXML_NOWARNING);
            $importedWrap = $dom->importNode($fragmentDoc->getElementById('ocd-mcp-fragment'), true);
        ...
        $root = $dom->getElementById('ocd-mcp-root');
        $new_html = '';
    ...
    /**
     * Reemplazo literal exacto dentro de _ocd_canvas_css. Exige que $search
    ...
    private function patch_css(int $page_id, string $document_id, int $expected_revision, string $search, string $replace)
    {
        ...
        if ($count === 0) {
            return new WP_Error('ocd_mcp_css_search_not_found', 'El texto buscado no aparece en el CSS de este documento.');
        }
        if ($count > 1) {
            return new WP_Error('ocd_mcp_css_search_ambiguous', 'El texto buscado aparece ' . $count . ' veces; debe ser único para aplicar el reemplazo con seguridad.');
        }
    ...
    private function read_canvas_document(int $page_id, string $document_id)
    {
        ...
        if ($described === null) {
            return new WP_Error('ocd_mcp_document_not_found', 'No existe un documento Canvas con ese ID.');
        }
        $document = $this->repository->load_existing($document_id);
        if (is_wp_error($document)) {
            return $document;
    ...
    private function write_canvas_document(int $page_id, string $document_id, int $expected_revision, string $project_data, string $html, string $css)
    {
        ...
        $sanitizer = new OCD_Canvas_Document_Sanitizer();
     ...
     * tools/ocd-headless-node-edit.mjs para el detalle. Este método solo
    ...
    private function grapes_edit_node(int $page_id, string $document_id, int $expected_revision, string $selector, array $mutation)
    {
        if (!function_exists('proc_open')) {
            return new WP_Error('ocd_mcp_grapes_no_proc_open', 'Este servidor tiene proc_open deshabilitado (revisar disable_functions en php.ini) — no puede ejecutar el puente headless a Grapes.');
        }
        ...
        if ($node_bin === null) {
            return new WP_Error('ocd_mcp_grapes_no_node', 'No se encontró el ejecutable de Node.js en este servidor. El puente headless a Grapes (tools/ocd-headless-node-edit.mjs) necesita Node instalado en la máquina donde corre el plugin.');
        }

        $script_path = OCD_PUBLISHER_DIR . 'tools/ocd-headless-node-edit.mjs';
        if (!file_exists($script_path)) {
            return new WP_Error('ocd_mcp_grapes_script_missing', 'No se encontró tools/ocd-headless-node-edit.mjs dentro del plugin.');
        }
        ...
        if (!is_string($payload)) {
            return new WP_Error('ocd_mcp_grapes_encode', 'No se pudo codificar el documento para el puente headless.');
        }
        ...
        if (empty($result['ok'])) {
            ...
            return new WP_Error('ocd_mcp_grapes_bridge_failed', $message);
        }
    ...
    private function run_headless_bridge(string $node_bin, string $script_path, string $payload)
    {
        ...
        if (!is_resource($process)) {
            return new WP_Error('ocd_mcp_grapes_spawn_failed', 'No se pudo iniciar el proceso Node para el puente headless.');
        }
        ...
        if ($stdout === '') {
            return new WP_Error('ocd_mcp_grapes_no_output', 'El puente headless no devolvió salida.' . ($stderr ? ' stderr: ' . $stderr : '') . ' (código de salida ' . $exit_code . ')');
        }
        $decoded = json_decode($stdout, true);
        if (!is_array($decoded)) {
            return new WP_Error('ocd_mcp_grapes_bad_output', 'El puente headless no devolvió JSON válido: ' . $stdout);
        }
        return $decoded;
    }
    ...
    private function sync_project_data(int $page_id, string $document_id, int $expected_revision)
    {
        ...
        if (!is_string($project_json)) {
            return new WP_Error('ocd_mcp_sync_project_encode', 'No se pudo codificar projectData.');
        }
        $sanitizer = new OCD_Canvas_Document_Sanitizer();
    ...
    private function patch_html(int $page_id, string $document_id, int $expected_revision, string $search, string $replace)
    {
        ...
        if ($count === 0) {
            return new WP_Error('ocd_mcp_html_search_not_found', 'El texto buscado no aparece en el HTML de este documento.');
        }
        if ($count > 1) {
            return new WP_Error('ocd_mcp_html_search_ambiguous', 'El texto buscado aparece ' . $count . ' veces; debe ser único para aplicar el reemplazo con seguridad.');
        }
    ...
    private function target(int $page_id, string $document_id, int $expected_revision)
    {
        ...
        //     resuelve por regla (ver OCD_Template_Region_Resolver) y por eso
        //     se valida contra list_region_documents();
        //   - el documento de estilos COMPARTIDOS entre todas las páginas
        //     (OCD_Canvas_Document_Repository::SHARED_STYLES_DOCUMENT_ID), que
        ...
        if ($page_id === 0) {
            $es_documento_global = hash_equals(OCD_Canvas_Document_Repository::SHARED_STYLES_DOCUMENT_ID, $document_id);
            ...
            if (!$es_documento_global) {
                return new WP_Error('ocd_mcp_canvas_page_not_found', 'pageId 0 sólo es válido para un documento de región global (header/footer/body) o para el documento de estilos compartidos; documentId no corresponde a ninguno.');
            }
            $document = $this->repository->describe_existing($document_id);
            if ($document === null) {
                return new WP_Error('ocd_mcp_canvas_document_missing', 'El documento Canvas indicado no está disponible.');
            }
        ...
        if (!hash_equals((string) $page['documentId'], $document_id)) {
            return new WP_Error('ocd_mcp_canvas_target_mismatch', 'documentId no corresponde a la página Canvas indicada.');
        }
        $document = $this->repository->describe_existing($document_id);
        if ($document === null) {
            return new WP_Error('ocd_mcp_canvas_document_missing', 'El documento Canvas indicado no está disponible.');
    ...
    private function target_arguments(array $arguments, array $extra_allowed)
    {
        ...
        if (!$this->has_only_keys($arguments, $allowed)
            ...
            return new WP_Error('ocd_mcp_invalid_arguments', 'pageId, documentId y expectedRevision deben identificar una revisión Canvas válida. pageId 0 = documento de región global (sin página).');
        }
    ...
    private function composition_arguments(array $arguments, bool $requires_preview_id)
    {
        ...
        if (!isset($arguments['composition']) || !is_array($arguments['composition']) || !isset($arguments['design']) || !is_array($arguments['design'])) {
            return new WP_Error('ocd_mcp_invalid_arguments', 'composition y design deben ser objetos JSON.');
        }
        if ($requires_preview_id) {
            if (!isset($arguments['previewId']) || !is_string($arguments['previewId']) || preg_match('/^[a-f0-9]{64}$/', $arguments['previewId']) !== 1) {
                return new WP_Error('ocd_mcp_invalid_arguments', 'previewId debe ser la evidencia SHA-256 devuelta por la previsualización.');
            }
    ...
    {
        return $arguments === [] ? null : new WP_Error('ocd_mcp_invalid_arguments', 'Esta herramienta no acepta argumentos.');
    }
    ...
    {
        if (!is_string($value)) {
            return new WP_Error('ocd_mcp_invalid_arguments', 'title debe ser texto.');
        }
        $title = sanitize_text_field($value);
        if ($title === '' || (function_exists('mb_strlen') ? mb_strlen($title) : strlen($title)) > 160) {
            return new WP_Error('ocd_mcp_invalid_arguments', 'title debe tener entre 1 y 160 caracteres.');
        }
        return $title;
    }
