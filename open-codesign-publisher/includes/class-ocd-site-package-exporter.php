 ...
 */
final class OCD_Site_Package_Exporter
{
    ...
    public function export(bool $include_media = true)
    {
        ...
        if ($pages === []) {
            return new WP_Error('ocd_package_empty', 'No hay páginas publicadas respaldadas por un documento Canvas.');
        }
        ...
        $default_template = $this->repository->get_template(OCD_Canvas_Document_Repository::DEFAULT_TEMPLATE_ID);
        $default_assignments = $this->repository->get_template_assignments(OCD_Canvas_Document_Repository::DEFAULT_TEMPLATE_ID);
        ...
        $manifest = [
            ...
            'pluginVersion' => defined('OCD_PUBLISHER_VERSION') ? OCD_PUBLISHER_VERSION : '',
        ...
        if ($json === false) {
            return new WP_Error('ocd_package_encode', 'No fue posible serializar el paquete.');
        }
    ...
    private function collect_pages(): array
    {
        ...
        foreach ($post_ids as $post_id) {
            $document_id = (string) get_post_meta((int) $post_id, '_ocd_canvas_document_id', true);
            if ($document_id === '') {
                continue;
            }
            $pages[] = [
    ...
    private function collect_site_font_paths(): array
    {
        ...
        $fonts_dir = trailingslashit((string) $upload_dir['basedir']) . 'open-codesign/fonts';
        if (!is_dir($fonts_dir)) {
            return [];
        }
        ...
        foreach (glob(trailingslashit($fonts_dir) . '*') as $absolute) {
            ...
            $found['open-codesign/fonts/' . basename($absolute)] = $absolute;
        }
    ...
    private function write_zip(string $manifest_json, array $media_paths)
    {
        ...
        $tmp_path = wp_tempnam('ocd-site-package.zip');
        if (!$tmp_path) {
            return new WP_Error('ocd_package_tmp', 'No fue posible crear un archivo temporal.');
        }

        $zip = new ZipArchive();
        if ($zip->open($tmp_path, ZipArchive::OVERWRITE) !== true) {
            return new WP_Error('ocd_package_zip_open', 'No fue posible abrir el ZIP para escritura.');
        }
