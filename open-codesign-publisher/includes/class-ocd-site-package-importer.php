...
/**
 * Importa un paquete generado por OCD_Site_Package_Exporter: recrea (o
 ...
 */
final class OCD_Site_Package_Importer
{
    ...
    public function import(string $zip_path)
    {
        if (!class_exists('ZipArchive')) {
            return new WP_Error('ocd_package_zip_unavailable', 'La extensión ZipArchive de PHP no está disponible.');
        }

        $zip = new ZipArchive();
        if ($zip->open($zip_path) !== true) {
            return new WP_Error('ocd_package_zip_open', 'No fue posible abrir el paquete ZIP.');
        }

        $manifest_json = $zip->getFromName('manifest.json');
        if ($manifest_json === false) {
            $zip->close();
            return new WP_Error('ocd_package_manifest_missing', 'El paquete no contiene manifest.json.');
        }
        ...
        try {
            ...
            $zip->close();
            return new WP_Error('ocd_package_manifest_invalid', 'manifest.json no es JSON válido.');
        }
        if (!is_array($manifest) || !isset($manifest['schemaVersion'], $manifest['pages'], $manifest['documents'])) {
            $zip->close();
            return new WP_Error('ocd_package_manifest_shape', 'manifest.json no tiene la forma esperada.');
        }
        if ((int) $manifest['schemaVersion'] !== OCD_Site_Package_Exporter::PACKAGE_SCHEMA_VERSION) {
            $zip->close();
            return new WP_Error('ocd_package_schema_mismatch', 'Versión de paquete no soportada por este plugin.');
        }
        ...
        if (is_array($assignments)) {
            foreach ($assignments as $kind => $document_id) {
                ...
                $assigned = $this->repository->assign_region_to_template(
                    OCD_Canvas_Document_Repository::DEFAULT_TEMPLATE_ID,
                    (string) $kind,
                    $document_id
                );
    ...
    private function import_document(string $document_id, $document, string $source_url, string $target_url)
    {
        if (!is_array($document)) {
            return new WP_Error('ocd_package_document_shape', "El documento $document_id no es un objeto.");
        }
    ...
    private function import_page($page)
    {
        if (!is_array($page) || !isset($page['documentId'], $page['title'], $page['slug'])) {
            return new WP_Error('ocd_package_page_shape', 'Una entrada de página no tiene la forma esperada.');
        }
        ...
        // necesita el shortcode que OCD_Canvas_Page_Publisher::publish()
        ...
        $post_arr = [
            ...
            'meta_input' => [
                '_ocd_canvas_document_id' => $document_id,
            ],
        ];
    ...
    private function copy_media(ZipArchive $zip)
    {
        ...
        if (!empty($upload_dir['error'])) {
            return new WP_Error('ocd_package_upload_dir', (string) $upload_dir['error']);
        }
        ...
        for ($i = 0; $i < $zip->numFiles; $i++) {
            ...
            if (!is_dir($target_dir) && !wp_mkdir_p($target_dir)) {
                return new WP_Error('ocd_package_media_dir', "No fue posible crear el directorio para $relative.");
            }

            $stream = $zip->getStream($name);
            if ($stream === false) {
                return new WP_Error('ocd_package_media_read', "No fue posible leer $relative del paquete.");
            }
            $out = fopen($target_path, 'wb');
            if ($out === false) {
                fclose($stream);
                return new WP_Error('ocd_package_media_write', "No fue posible escribir $relative.");
            }
